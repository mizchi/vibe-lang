...
- **ビット演算子**: `&`, `|`, `^`, `<<`, `>>`, および前置 `~` (bit-not, #2344 slice C)。
  `~x` は 63-bit 二の補数上の補数で、`~x == -x - 1` (`~0 == -1`, `~5 == -6`)。
  lowering は `x ^ -1` だが **定数はレーン依存**: RC レーンでは `Int` が `n<<1`
  なので tagged(-1) = `-2` と xor する。生の `-1` と xor するとタグビットが
  立ち、非負の小さい値では正しく見えて符号をまたぐと壊れる —
  `fixtures/bit_not_test.vibe` が負の入力を必ず含むのはそのため
