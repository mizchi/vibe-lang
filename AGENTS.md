...
source) disproved for a string literal. Measured 2026-09-18 on this branch's
stage2, identically on the FS lane and under `--single-file`:
...
So what remains is narrower than "no range, wrong anchor", and splits in two:

1. **Literals other than `String` carry no offset slot** (`EInt(Int)` /
   `EFloat(Double)` / `EBool(Bool)` against `EString(String, Int)` and
   `EIdent(String, Int)`), so the binder anchors them and there is no end.
2. **The ARGUMENT path anchors the callee** even when the argument IS a string
   literal that has an offset -- `takes("nope")` reports `takes`, not `"nope"`.
   That one is not a missing slot; it is a call site that does not read the one
   it has.

The anchor machinery itself (`off_marker` / `railway_expr_off`) works, and
`check_assignable_set_end` plus `string_token_end` are how an end is supplied
and then corrected to the lexer's span.
