...
fn extract_function_signature(typ: &xs_core::Type) -> Option<(Vec<(String, WitType)>, Vec<WitType>)> {
    use xs_core::Type;
    ...
    loop {
        match current_type {
            ...
            Type::FunctionWithEffect { from: param_type, to: result_type, .. } => {
    ...
    for type_def in &interface.types {
        match type_def {
            ...
            TypeDefinition::Alias { name, target } => {
                wit.push_str(&format!("  type {} = {};\n\n", name, wit_type_to_string(target)));
            }
        }
    }
    ...
    for func in &interface.functions {
        ...
        if !func.results.is_empty() {
            ...
            if func.results.len() == 1 {
                ...
                let results: Vec<String> = func.results.iter()
                    .map(wit_type_to_string)
                    .collect();
