...
use xs_core::{Type, Ident, Span, XsError};
use xs_core::error_context::{ErrorBuilder, ErrorCategory, Suggestion, SuggestionConfidence};
...
impl TypeErrorHelper {
    ...
    pub fn type_mismatch(&self, expected: &Type, actual: &Type, span: Span) -> XsError {
        let context = ErrorBuilder::type_mismatch(expected.clone(), actual.clone())
            .with_snippet("", span) // Source will be added later
    ...
    pub fn undefined_variable(&self, name: &str, span: Span) -> XsError {
        let similar = self.find_similar_names(name);
        let mut builder = ErrorBuilder::undefined_variable(name)
            .with_similar_names(name, similar.clone());
    ...
    fn suggest_type_conversion(&self, from: &Type, to: &Type) -> String {
        match (from, to) {
            (Type::String, Type::Int) => "Convert string to integer using 'int_of_string'".to_string(),
            (Type::Int, Type::String) => "Convert integer to string using 'string_of_int'".to_string(),
            ...
            (Type::Bool, Type::String) => "Convert boolean to string using 'string_of_bool'".to_string(),
            _ => format!("Type '{}' cannot be used where '{}' is expected", 
    ...
    fn type_to_readable(&self, ty: &Type) -> String {
        match ty {
            ...
            Type::Function(a, b) => format!("function from {} to {}", 
            ...
            Type::Constructor(name, args) => {
                if args.is_empty() {
                    ...
                    format!("{} with {} type parameter(s)", name, args.len())
                }
            }
        }
    }
    ...
    fn find_similar_names(&self, name: &str) -> Vec<String> {
        let mut candidates: Vec<(String, usize)> = self.known_names.iter()
            .filter_map(|known| {
...
pub fn pattern_error_helper(pattern_type: &Type, value_type: &Type, span: Span) -> XsError {
    let mut builder = ErrorBuilder::new(
        ...
        format!("Pattern expects type '{}' but value has type '{}'", 
                type_string(pattern_type), type_string(value_type))
    );

    // Add specific suggestions for common pattern mistakes
    match (pattern_type, value_type) {
        (Type::List(_), Type::Constructor(name, _)) if name == "Nil" || name == "Cons" => {
        ...
        (Type::Constructor(p_name, _), Type::Constructor(v_name, _)) if p_name != v_name => {
            builder = builder.suggest(
                format!("Expected constructor '{}' but found '{}'", p_name, v_name),
                None
            );
        }
        _ => {}
    ...
    match ty {
        ...
        Type::Constructor(name, args) => {
            if args.is_empty() {
                ...
                format!("({} {})", name, args.iter().map(type_string).collect::<Vec<_>>().join(" "))
            }
        }
    }
}
...
mod tests {
    ...
    fn test_type_mismatch_suggestions() {
        ...
        let error = helper.type_mismatch(&Type::Int, &Type::String, Span::new(0, 10));
    ...
    fn test_similar_name_suggestions() {
        let helper = TypeErrorHelper::new()
            .with_known_names(vec!["map".to_string(), "filter".to_string(), "fold".to_string()]);
    ...
    fn test_levenshtein_distance() {
        assert_eq!(levenshtein_distance("map", "mpa"), 1);
