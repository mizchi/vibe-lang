...
fn generate_polymorphic_functions(count: usize) -> String {
    ...
    for i in 0..count {
        functions.push(format!("(let id{} (lambda (x) x)", i));
    }
...
fn benchmark_incremental_type_checking(c: &mut Criterion) {
    ...
    group.bench_function("incremental_with_salsa", |b| {
        b.iter(|| {
            let mut db = XsSalsaDatabase::default();
...
fn benchmark_type_instantiation(c: &mut Criterion) {
    ...
    for count in [10, 50, 100, 200].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("instantiations", count),
            count,
            |b, _| {
                b.iter(|| {
                    let mut checker = TypeChecker::new();
...
fn benchmark_recursive_type_checking(c: &mut Criterion) {
    let programs = vec![
        ("simple_rec", r#"(rec fact (lambda (n) (if (= n 0) 1 (* n (fact (- n 1))))))"#),
        ("mutual_rec", r#"
            (let-rec even (lambda (n) (if (= n 0) true (odd (- n 1))))
            (let-rec odd (lambda (n) (if (= n 0) false (even (- n 1))))
                (even 10)))
        "#),
        ("complex_rec", r#"
            (rec fib (lambda (n) 
