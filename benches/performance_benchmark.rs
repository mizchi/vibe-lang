...
fn benchmark_parser(c: &mut Criterion) {
    let simple_expr = "(+ 1 2)";
    let complex_expr = r#"(let f (fn (x) 
        (let g (fn (y) (+ x y))
            (g (+ x 1))))
        (f 10))"#;

    c.bench_function("parse_simple", |b| b.iter(|| parse(black_box(simple_expr))));
...
fn benchmark_interpreter(c: &mut Criterion) {
    ...
    c.bench_function("eval_simple", |b| b.iter(|| eval(black_box(&simple_ast))));

    c.bench_function("eval_complex", |b| b.iter(|| eval(black_box(&complex_ast))));
}

fn benchmark_full_pipeline(c: &mut Criterion) {
    let program = r#"(let double (fn (x) (* x 2))
