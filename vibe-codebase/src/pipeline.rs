            ...
            StructuredData::Table { columns, mut rows } => {
                rows.sort_by(|a, b| {
                    ...
                    let ordering = match (a_val, b_val) {
                        (Some(StructuredValue::String(s1)), Some(StructuredValue::String(s2))) => {
                            s1.cmp(s2)
                        }
                        (Some(StructuredValue::Int(n1)), Some(StructuredValue::Int(n2))) => {
                            n1.cmp(n2)
                        }
...
impl PipelineOperator for TakeOperator {
    fn apply(&self, input: StructuredData) -> Result<StructuredData, XsError> {
        match input {
            StructuredData::Table { columns, rows } => {
                let taken: Vec<_> = rows.into_iter().take(self.count).collect();
                Ok(StructuredData::Table {
                    columns,
                    rows: taken,
                })
            ...
            StructuredData::Definitions(defs) => {
                let taken: Vec<_> = defs.into_iter().take(self.count).collect();
                Ok(StructuredData::Definitions(taken))
            }

            StructuredData::List(items) => {
                let taken: Vec<_> = items.into_iter().take(self.count).collect();
        ...
        match input {
            StructuredData::Table { columns: _, rows } => {
                ...
                for (key, group_rows) in groups {
                    ...
                    row.insert(
                        "count".to_string(),
                        StructuredValue::Int(group_rows.len() as i64),
                    );
...
pub fn parse_pipeline_operator(cmd: &str) -> Result<Box<dyn PipelineOperator>, XsError> {
    ...
    match parts[0] {
        ...
        "select" => {
            ...
            let columns: Vec<String> = parts[1..].iter().map(|s| s.to_string()).collect();
        ...
        "take" => {
            ...
            let count = parts[1].parse::<usize>().map_err(|_| {
                XsError::RuntimeError(Span::new(0, 0), format!("Invalid count: {}", parts[1]))
            })?;
        ...
        "count" => Ok(Box::new(CountOperator)),
