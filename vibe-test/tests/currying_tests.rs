...
fn test_automatic_currying_lambda() {
    ...
    fs::write("test_auto_curry.vibe", code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_auto_curry.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("closure")); // Partial application returns a closure

    fs::remove_file("test_auto_curry.vibe").ok();
}
...
fn test_automatic_currying_application() {
    ...
    fs::write("test_curry_app.vibe", code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_curry_app.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("12"));

    fs::remove_file("test_curry_app.vibe").ok();
}
...
fn test_three_parameter_currying() {
    ...
    fs::write("test_three_curry.vibe", code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_three_curry.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("17")); // 3 * 4 + 5 = 17

    fs::remove_file("test_three_curry.vibe").ok();
}
...
fn test_builtin_currying() {
    ...
    let code = r#"10 + 20"#;
    fs::write("test_builtin_curry.vibe", code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_builtin_curry.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("30"));

    fs::remove_file("test_builtin_curry.vibe").ok();
}
...
fn test_partial_application_binding() {
    ...
    fs::write("test_partial_bind.vibe", code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_partial_bind.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("closure") || stdout.contains("<builtin:+>"));

    fs::remove_file("test_partial_bind.vibe").ok();
}
...
fn test_composition_with_currying() {
    ...
    fs::write("test_curry_compose.vibe", code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_curry_compose.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("closure")); // Returns composed function

    fs::remove_file("test_curry_compose.vibe").ok();
}
...
fn test_curried_recursive_function() {
    ...
    fs::write("test_rec_curry.vibe", code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_rec_curry.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("closure")); // Partial application of recursive function

    fs::remove_file("test_rec_curry.vibe").ok();
}
...
fn test_higher_order_currying() {
    ...
    fs::write("test_ho_curry.vibe", code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_ho_curry.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("7"));

    fs::remove_file("test_ho_curry.vibe").ok();
}
...
fn test_curried_type_checking() {
    ...
    let code = r#"(fn x = fn y = x + y) 5"#;
    fs::write("test_curry_types.vibe", code).unwrap();

    let (_, stderr, success) = run_xsc(&["check", "test_curry_types.vibe"]);
    assert!(success, "Type check failed: {stderr}");

    fs::remove_file("test_curry_types.vibe").ok();
}
...
fn test_curried_list_operations() {
    ...
    let code = r#"[1, 2, 3]"#;
    fs::write("test_curry_list.vibe", code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_curry_list.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("(list 1 2 3)"));

    fs::remove_file("test_curry_list.vibe").ok();
}
