...
(module Math
  ...
  (let multiply (fn (x y) (* x y))))"#;
    ...
    let module_path = temp_dir.path().join("math.vibe");
...
(module Curried
  ...
  (let curry-multiply (fn (x) (fn (y) (fn (z) (* (* x y) z))))))"#;
    ...
    let temp_dir = TempDir::new().unwrap();
    let module_path = temp_dir.path().join("curried.vibe");
    fs::write(&module_path, module_content).unwrap();
...
(module StringUtils
  ...
  (let identity (fn (s) s)) ; Returns input string
    ...
    let temp_dir = TempDir::new().unwrap();
    let module_path = temp_dir.path().join("string_utils.vibe");
    fs::write(&module_path, module_content).unwrap();
...
(module ListOps
  ...
  (let map-list (fn (f xs) xs))
    ...
    let temp_dir = TempDir::new().unwrap();
    let module_path = temp_dir.path().join("list_ops.vibe");
    fs::write(&module_path, module_content).unwrap();
...
(module FloatMath
  ...
  (let floor (fn (x: Float) 1.0))
    ...
    let temp_dir = TempDir::new().unwrap();
    let module_path = temp_dir.path().join("float_math.vibe");
    fs::write(&module_path, module_content).unwrap();
...
(module TestModule
  ...
  (let test-func (fn (x) (* x 2))))"#;
    ...
    let temp_dir = TempDir::new().unwrap();
    let module_path = temp_dir.path().join("test.vibe");
...
(module Empty
  ...
  ; No exports
    ...
    let temp_dir = TempDir::new().unwrap();
    let module_path = temp_dir.path().join("empty.vibe");
    fs::write(&module_path, module_content).unwrap();
...
fn test_non_module_expression_wit_generation() {
    ...
    let temp_dir = TempDir::new().unwrap();
    let expr_path = temp_dir.path().join("expr.vibe");
...
fn test_invalid_syntax_error() {
    ...
    let temp_dir = TempDir::new().unwrap();
    let invalid_path = temp_dir.path().join("invalid.vibe");
...
(module TypeErr
  ...
  ; Referencing undefined function
    ...
    let temp_dir = TempDir::new().unwrap();
    let module_path = temp_dir.path().join("type_err.vibe");
    fs::write(&module_path, module_content).unwrap();
