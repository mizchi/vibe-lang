...
(module StringOps
  ...
  (let str-length (fn (s) (string-length s))))"#;
        ...
        let (result,) = length_func.call(&mut store, ("Vibe Language",)).unwrap();
        assert_eq!(result, 11);
    }
}
...
(module ValidWIT
  ...
  (let calculate (fn (a b c) (+ (* a b) c))))"#;
    ...
    let module_path = temp_dir.path().join("valid.vibe");
...
(module Metadata
  ...
  (let get-info (fn (key) "value")))"#;
    ...
    let temp_dir = TempDir::new().unwrap();
    let module_path = temp_dir.path().join("metadata.vibe");
    fs::write(&module_path, module_content).unwrap();
...
(module DataConsumer
  ...
  (let consume (fn (provider-func id) (provider-func id))))"#;
    ...
    let provider_path = temp_dir.path().join("provider.vibe");
    ...
    let consumer_path = temp_dir.path().join("consumer.vibe");
...
(module EdgeCases
  ...
  (let test_underscore (fn (a b) (+ a b))))"#;
    ...
    let temp_dir = TempDir::new().unwrap();
    let module_path = temp_dir.path().join("edge_cases.vibe");
    fs::write(&module_path, module_content).unwrap();
