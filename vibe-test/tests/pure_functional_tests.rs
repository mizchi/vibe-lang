//! Tests for pure functional programming features and currying
//! 
//! NOTE: These tests use S-expression syntax and need to be updated for the new Haskell-style parser

#![cfg(skip)]  // Skip these tests until they are updated

use std::fs;
use std::process::Command;

fn run_xsc(args: &[&str]) -> (String, String, bool) {
    let output = Command::new("cargo")
        .args(["run", "-p", "vsh", "--bin", "vsh", "--"])
        .args(args)
        .output()
        .expect("Failed to execute xsc");

    let stdout = String::from_utf8_lossy(&output.stdout).to_string();
    let stderr = String::from_utf8_lossy(&output.stderr).to_string();

    (stdout, stderr, output.status.success())
}

#[test]
fn test_automatic_currying() {
    // Test partial application (manual currying in XS)
    let curry_code = r#"((fn (x) (fn (y) (+ x y))) 5)"#;
    fs::write("test_curry.vibe", curry_code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_curry.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("closure"));

    fs::remove_file("test_curry.vibe").ok();
}

#[test]
fn test_higher_order_functions() {
    // Test higher-order functions with simple example
    let hof_code = r#"((fn (f) (f 10)) (fn (x) (* x 2)))"#;
    fs::write("test_hof.vibe", hof_code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_hof.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("20"));

    fs::remove_file("test_hof.vibe").ok();
}

#[test]
fn test_function_composition() {
    // Test function composition
    let compose_code = r#"((fn (f g) (fn (x) (f (g x)))) (fn (x) (+ x 1)) (fn (x) (* x 2)))"#;
    fs::write("test_compose.vibe", compose_code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_compose.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("closure")); // Returns a composed function

    fs::remove_file("test_compose.vibe").ok();
}

#[test]
fn test_simple_recursion() {
    // Test simple recursive function
    let rec_code = r#"((rec sum (n) (if (= n 0) 0 (+ n (sum (- n 1))))) 5)"#;
    fs::write("test_simple_rec.vibe", rec_code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_simple_rec.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("15")); // 1+2+3+4+5 = 15

    fs::remove_file("test_simple_rec.vibe").ok();
}

#[test]
fn test_pure_function_property() {
    // Test that the same input always produces the same output
    let pure_code = r#"(= ((fn (x y) (+ x y)) 5 3) ((fn (x y) (+ x y)) 5 3))"#;
    fs::write("test_pure.vibe", pure_code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_pure.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("true"));

    fs::remove_file("test_pure.vibe").ok();
}

#[test]
fn test_immutable_data() {
    // Test that data structures are immutable
    let immutable_code = r#"(cons 0 (list 1 2 3))"#;
    fs::write("test_immutable.vibe", immutable_code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_immutable.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("(list 0 1 2 3)"));

    fs::remove_file("test_immutable.vibe").ok();
}

#[test]
fn test_tail_recursion() {
    // Test tail-recursive implementation
    let tail_rec_code =
        r#"((rec sum_tail (n acc) (if (= n 0) acc (sum_tail (- n 1) (+ acc n)))) 100 0)"#;
    fs::write("test_tail_rec.vibe", tail_rec_code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_tail_rec.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("5050")); // Sum of 1 to 100

    fs::remove_file("test_tail_rec.vibe").ok();
}

#[test]
fn test_lazy_evaluation_simulation() {
    // Simulate lazy evaluation with thunks
    let lazy_code = r#"((fn (t) (t)) (fn () (+ 1 2)))"#;
    fs::write("test_lazy.vibe", lazy_code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_lazy.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("3"));

    fs::remove_file("test_lazy.vibe").ok();
}

#[test]
fn test_referential_transparency() {
    // Test that expressions can be replaced by their values
    let ref_trans_code = r#"(= (+ 5 5) 10)"#;
    fs::write("test_ref_trans.vibe", ref_trans_code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_ref_trans.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("true"));

    fs::remove_file("test_ref_trans.vibe").ok();
}

#[test]
fn test_list_manipulation() {
    // Test list manipulation
    let list_code = r#"(match (list 1 2 3) ((list h t s) h) ((list) 0))"#;
    fs::write("test_list_manip.vibe", list_code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_list_manip.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("1"));

    fs::remove_file("test_list_manip.vibe").ok();
}

#[test]
fn test_algebraic_data_types() {
    // Test ADT definition and usage (check only)
    let adt_code = r#"(type Option a (None) (Some a))"#;
    fs::write("test_adt.vibe", adt_code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["parse", "test_adt.vibe"]);
    assert!(success, "Parse failed: {stderr}");
    assert!(stdout.contains("TypeDef"));

    fs::remove_file("test_adt.vibe").ok();
}

#[test]
fn test_nested_pattern_matching() {
    // Test nested pattern matching - skip for now due to complex pattern syntax
    // TODO: Re-enable when pattern matching syntax is stabilized
    return;
    
    /*
    let pattern_code = r#"(match (list 1 (list 2 3)) ((list h t) (match t ((list x ... xs) x) (_ 0))) (_ 0))"#;
    fs::write("test_nested_pattern.vibe", pattern_code).unwrap();

    let (stdout, stderr, success) = run_xsc(&["exec", "test_nested_pattern.vibe"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("2"));

    fs::remove_file("test_nested_pattern.vibe").ok();
    */
}
