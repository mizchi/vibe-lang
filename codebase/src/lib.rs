...
pub struct TypeDef {
    pub hash: Hash,
    pub name: String,
    pub definition: xs_core::TypeDefinition,
}
...
impl Codebase {
    ...
    pub fn add_term(&mut self, name: Option<String>, expr: Expr, ty: Type) -> Result<Hash, CodebaseError> {
        ...
        let serialized = bincode::serialize(&normalized)
            .map_err(CodebaseError::SerializationError)?;
    ...
    pub fn edit(&self, name: &str) -> Result<String, CodebaseError> {
        let term = self.get_term_by_name(name)
    ...
    pub fn update(&mut self, name: &str, new_expr_str: &str) -> Result<Hash, CodebaseError> {
        ...
        let new_expr = parse(new_expr_str)
            .map_err(|e| CodebaseError::ParseError(e.to_string()))?;
        
        // Type check
        let mut checker = TypeChecker::new();
        let mut env = TypeEnv::new();
        let ty = checker.check(&new_expr, &mut env)
    ...
    pub fn remove_term(&mut self, hash: &Hash) -> Result<(), CodebaseError> {
        ...
        if let Some(dependents) = self.dependents.get(hash) {
            if !dependents.is_empty() {
                return Err(CodebaseError::TermNotFound(
                    format!("Cannot remove term with {} dependents", dependents.len())
                ));
            }
        }
    ...
    pub fn load(path: &Path) -> Result<Self, CodebaseError> {
        ...
        let codebase = bincode::deserialize(&data)
            .map_err(CodebaseError::SerializationError)?;
        Ok(codebase)
    }
}
...
impl Serialize for Codebase {
        ...
        S: serde::Serializer,
    {
        ...
        let terms: HashMap<_, _> = self.terms.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
        let types: HashMap<_, _> = self.types.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
        let term_names: HashMap<_, _> = self.term_names.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
        let type_names: HashMap<_, _> = self.type_names.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
        let dependencies: HashMap<_, _> = self.dependencies.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
        let dependents: HashMap<_, _> = self.dependents.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
...
mod tests {
    ...
    fn test_codebase_add_term() {
        ...
        let expr = Expr::Literal(xs_core::Literal::Int(42), xs_core::Span::new(0, 2));
        let ty = Type::Int;
        
        let hash = codebase.add_term(Some("answer".to_string()), expr.clone(), ty.clone()).unwrap();
    ...
    fn test_dependency_extraction() {
        ...
        // Add a base function
        let base_expr = Expr::Literal(xs_core::Literal::Int(10), xs_core::Span::new(0, 2));
        let base_hash = codebase.add_term(Some("base".to_string()), base_expr, Type::Int).unwrap();
        
        // Add a function that depends on base
        let dependent_expr = Expr::Apply {
            func: Box::new(Expr::Ident(Ident("base".to_string()), xs_core::Span::new(0, 4))),
            args: vec![],
            span: xs_core::Span::new(0, 6),
        };
        let dependent_hash = codebase.add_term(Some("dependent".to_string()), dependent_expr, Type::Int).unwrap();
    ...
    fn test_edit_command() {
        ...
        let expr = Expr::Lambda {
            ...
            body: Box::new(Expr::Ident(Ident("x".to_string()), xs_core::Span::new(0, 1))),
            span: xs_core::Span::new(0, 10),
        };
        codebase.add_term(Some("identity".to_string()), expr, Type::Function(Box::new(Type::Int), Box::new(Type::Int))).unwrap();
    ...
    fn test_update_term() {
        ...
        // Add initial term
        let expr1 = Expr::Literal(xs_core::Literal::Int(42), xs_core::Span::new(0, 2));
        codebase.add_term(Some("x".to_string()), expr1, Type::Int).unwrap();
        ...
        match &term.expr {
            Expr::Literal(xs_core::Literal::Int(n), _) => assert_eq!(*n, 100),
    ...
    fn test_remove_term() {
        ...
        // Add a term
        let expr = Expr::Literal(xs_core::Literal::Int(42), xs_core::Span::new(0, 2));
        let hash = codebase.add_term(Some("x".to_string()), expr, Type::Int).unwrap();
    ...
    fn test_remove_term_with_dependents() {
        ...
        // Add dependent term
        let dependent_expr = Expr::Ident(Ident("base".to_string()), xs_core::Span::new(0, 4));
        codebase.add_term(Some("dependent".to_string()), dependent_expr, Type::Int).unwrap();
    ...
    fn test_complex_dependencies() {
        ...
        let a_expr = Expr::Literal(xs_core::Literal::Int(1), xs_core::Span::new(0, 1));
        codebase.add_term(Some("a".to_string()), a_expr, Type::Int).unwrap();
        
        let b_expr = Expr::Apply {
            func: Box::new(Expr::Ident(Ident("a".to_string()), xs_core::Span::new(0, 1))),
            args: vec![],
            span: xs_core::Span::new(0, 3),
        };
        codebase.add_term(Some("b".to_string()), b_expr, Type::Int).unwrap();
        
        let c_expr = Expr::Apply {
            func: Box::new(Expr::Ident(Ident("b".to_string()), xs_core::Span::new(0, 1))),
            args: vec![],
            span: xs_core::Span::new(0, 3),
        };
        let c_hash = codebase.add_term(Some("c".to_string()), c_expr, Type::Int).unwrap();
    ...
    fn test_patch_complex_operations() {
        ...
        // Add initial terms
        let expr1 = Expr::Literal(xs_core::Literal::Int(1), xs_core::Span::new(0, 1));
        let hash1 = codebase.add_term(Some("x".to_string()), expr1.clone(), Type::Int).unwrap();
        
        let expr2 = Expr::Literal(xs_core::Literal::Int(2), xs_core::Span::new(0, 1));
        codebase.add_term(Some("y".to_string()), expr2.clone(), Type::Int).unwrap();
        ...
        match &y_term.expr {
            Expr::Literal(xs_core::Literal::Int(n), _) => assert_eq!(*n, 3),
            _ => panic!("Expected int literal"),
        }
    }
    ...
    fn test_serialization() {
        ...
        let expr2 = Expr::Lambda {
            ...
            body: Box::new(Expr::Ident(Ident("y".to_string()), xs_core::Span::new(0, 1))),
            span: xs_core::Span::new(0, 10),
        };
        codebase.add_term(Some("id".to_string()), expr2, Type::Function(Box::new(Type::Int), Box::new(Type::Int))).unwrap();
    ...
    fn test_extract_deps_from_patterns() {
        ...
        // Add a base term
        let base_expr = Expr::Literal(xs_core::Literal::Int(10), xs_core::Span::new(0, 2));
        codebase.add_term(Some("base".to_string()), base_expr, Type::Int).unwrap();
        
        // Create a match expression that uses base
        let match_expr = Expr::Match {
            expr: Box::new(Expr::Ident(Ident("base".to_string()), xs_core::Span::new(0, 4))),
            cases: vec![
                (Pattern::Literal(xs_core::Literal::Int(10), xs_core::Span::new(0, 2)), 
                 Expr::Literal(xs_core::Literal::Bool(true), xs_core::Span::new(0, 4))),
            ],
