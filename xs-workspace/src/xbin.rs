//! XBin - XS言語のコンパクトなバイナリコードベースフォーマット
//! 
...
/// XBinストレージ - 効率的なバイナリコードベース管理
pub struct XBinStorage {
    path: String,
    /// メモリ内インデックスキャッシュ
...
impl XBinStorage {
    ...
    pub fn save_full(&mut self, codebase: &Codebase) -> Result<(), String> {
        let mut file = File::create(&self.path)
            .map_err(|e| format!("Failed to create xbin file: {}", e))?;
    ...
    fn ensure_index_loaded(&mut self) -> Result<(), String> {
        ...
        let mut file = File::open(&self.path)
            .map_err(|e| format!("Failed to open xbin file: {}", e))?;
        ...
        if magic != [b'X', b'B', b'I', b'N'] {
            return Err(format!("Invalid xbin file format. Expected {:?}, got {:?}", 
                [b'X', b'B', b'I', b'N'], magic));
        }
        
        if version != XBIN_VERSION {
            return Err(format!("Unsupported xbin version: {}", version));
        }
    ...
    /// メタデータを読み込み
    fn read_metadata(&self) -> Result<XBinMetadata, String> {
        let mut file = File::open(&self.path)
            .map_err(|e| format!("Failed to open xbin file: {}", e))?;
...
impl XBinStorage {
    /// 差分を適用してXBinファイルを更新
    pub fn apply_delta(&mut self, _delta: &XBinDelta) -> Result<(), String> {
        // TODO: 差分適用の実装
        Ok(())
    }
    ...
    /// 統計情報を取得
    pub fn stats(&mut self) -> Result<XBinStats, String> {
        self.ensure_index_loaded()?;
        let metadata = self.read_metadata()?;
        let index = self.index_cache.as_ref().unwrap();
