...
use xs_core::block_attributes::{BlockAttributeRegistry, BlockAttributes};
use xs_core::metadata::NodeId;
...
impl CodebaseBlockRegistry {
    ...
    pub fn store_attributes(&mut self, expr_hash: String, registry: BlockAttributeRegistry) {
        // Store the registry
        self.expr_attributes.insert(expr_hash.clone(), registry.clone());
...
pub trait CodebaseWithBlockAttributes {
    ...
    fn analyze_and_store(&mut self, expr_hash: String, expr: &xs_core::Expr) -> Result<(), String>;
...
mod tests {
    use super::*;
    use xs_core::block_attributes::EffectPermissions;
