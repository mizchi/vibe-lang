...
impl CodeRepository {
    ...
    /// トランザクション内で項を格納
    fn store_term_tx(tx: &Transaction, term: &Term, dependencies: &HashSet<Hash>) -> Result<(), String> {
        let content = bincode::serialize(term)
            .map_err(|e| format!("Failed to serialize term: {}", e))?;
        
        let namespace = term.name.as_ref()
    ...
    pub fn store_type(&mut self, type_def: &TypeDef) -> Result<(), String> {
        let content = bincode::serialize(type_def)
    ...
    /// 評価を記録
    pub fn record_evaluation(&mut self, input: &str, result_hash: Option<&Hash>, result_value: &str) -> Result<(), String> {
    ...
    pub fn analyze_reachability(&self, root_namespaces: &[String]) -> Result<ReachabilityAnalysis, String> {
        // ルートとなる定義を取得
        ...
        for ns in root_namespaces {
            ...
            let hashes = stmt.query_map(params![ns], |row| {
        ...
        while let Some(hash) = queue.pop() {
            ...
            // この定義が依存している定義を取得
            let mut stmt = self.conn.prepare(
                "SELECT to_hash FROM dependencies WHERE from_hash = ?1"
            ).map_err(|e| format!("Failed to prepare statement: {}", e))?;
            
            let deps = stmt.query_map(params![hash.to_hex()], |row| {
                Ok(Hash::from_hex(&row.get::<_, String>(0)?).unwrap())
        ...
        let mut stmt = self.conn.prepare(
            "SELECT hash FROM definitions"
        ).map_err(|e| format!("Failed to prepare statement: {}", e))?;
        
        let all_hashes = stmt.query_map([], |row| {
            Ok(Hash::from_hex(&row.get::<_, String>(0)?).unwrap())
    ...
    pub fn get_definition(&self, hash: &Hash) -> Result<Option<Term>, String> {
        let mut stmt = self.conn.prepare(
            "SELECT content FROM definitions WHERE hash = ?1 AND kind = 'term'"
        ).map_err(|e| format!("Failed to prepare statement: {}", e))?;
        
        let mut rows = stmt.query_map(params![hash.to_hex()], |row| {
    ...
    /// 名前で定義を検索
    pub fn search_by_name(&self, pattern: &str) -> Result<Vec<(Hash, String, DefinitionKind)>, String> {
        let mut stmt = self.conn.prepare(
            "SELECT hash, name, kind FROM definitions WHERE name LIKE ?1"
        ).map_err(|e| format!("Failed to prepare statement: {}", e))?;
        
        let results = stmt.query_map(params![format!("%{}%", pattern)], |row| {
...
mod tests {
    ...
    fn test_store_and_retrieve_term() {
        ...
        // Create a test term
        let expr = xs_core::Expr::Literal(xs_core::Literal::Int(42), xs_core::Span::new(0, 2));
        let hash = Hash::from_hex("1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef").unwrap();
        let term = Term {
            hash: hash.clone(),
    ...
    fn test_dependencies() {
        ...
        let hash1 = Hash::from_hex("1111111111111111111111111111111111111111111111111111111111111111").unwrap();
        let hash2 = Hash::from_hex("2222222222222222222222222222222222222222222222222222222222222222").unwrap();
        
        let term1 = Term {
            hash: hash1.clone(),
            name: Some("base".to_string()),
            expr: xs_core::Expr::Literal(xs_core::Literal::Int(1), xs_core::Span::new(0, 1)),
            ty: xs_core::Type::Int,
            dependencies: HashSet::new(),
        };
        ...
        let term2 = Term {
            ...
            expr: xs_core::Expr::Ident(xs_core::Ident("base".to_string()), xs_core::Span::new(0, 4)),
            ty: xs_core::Type::Int,
    ...
    fn test_search_by_name() {
        ...
        // Add some terms
        let terms = vec![
            ("fooBar", "1111111111111111111111111111111111111111111111111111111111111111"),
            ("fooBaz", "2222222222222222222222222222222222222222222222222222222222222222"),
            ("barFoo", "3333333333333333333333333333333333333333333333333333333333333333"),
        ];
    ...
    fn test_reachability_analysis() {
        ...
        let add_hash = Hash::from_hex("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa").unwrap();
        let double_hash = Hash::from_hex("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb").unwrap();
        let compute_hash = Hash::from_hex("cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc").unwrap();
        let unused_hash = Hash::from_hex("dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd").unwrap();
        ...
        let double_term = Term {
            ...
            expr: xs_core::Expr::Literal(xs_core::Literal::Int(2), xs_core::Span::new(0, 1)),
            ty: xs_core::Type::Int,
        ...
        let compute_term = Term {
            ...
            expr: xs_core::Expr::Literal(xs_core::Literal::Int(3), xs_core::Span::new(0, 1)),
            ty: xs_core::Type::Int,
        ...
        let unused_term = Term {
            ...
            expr: xs_core::Expr::Literal(xs_core::Literal::Int(4), xs_core::Span::new(0, 1)),
            ty: xs_core::Type::Int,
    ...
    fn test_dead_code_removal() {
        ...
        let dead_hash = Hash::from_hex("deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef").unwrap();
        let dead_term = Term {
            hash: dead_hash.clone(),
            name: Some("dead_code".to_string()),
            expr: xs_core::Expr::Literal(xs_core::Literal::Int(666), xs_core::Span::new(0, 3)),
            ty: xs_core::Type::Int,
            dependencies: HashSet::new(),
        };
