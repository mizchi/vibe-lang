...
use xs_core::{Expr, Value, Environment};
...
impl TestRunner {
    ...
    fn execute_test(&self, expr: &Expr) -> (TestOutcome, Duration) {
        ...
        let mut type_env = xs_compiler::TypeEnv::default();
    ...
    fn populate_type_env(&self, env: &mut xs_compiler::TypeEnv) {
        for (name, hash) in &self.codebase.term_names {
            if let Some(term) = self.codebase.get_term(hash) {
                env.add_binding(
                    name.clone(),
                    xs_compiler::TypeScheme::mono(term.ty.clone()),
                );
            }
        }
    }
