//! Incremental Type Checker for XS Language
//! 
...
use crate::namespace::{NamespaceStore, DefinitionPath};
...
impl IncrementalTypeChecker {
    ...
    pub fn type_check_definition(&mut self, path: &DefinitionPath) -> Result<Type, XsError> {
        let definition = self.namespace_store
        ...
        if let Some(cached) = self.cache.get(&hash) {
            if cached.success {
                ...
                return Err(XsError::TypeError(
                    xs_core::Span::new(0, 0),
                    cached.error.clone().unwrap_or_else(|| "Type check failed".to_string())
                ));
            }
        }
    ...
    fn collect_dependencies_to_check(&self, hash: &DefinitionHash) -> Result<Vec<DefinitionHash>, XsError> {
        ...
        match &definition.content {
            crate::namespace::DefinitionContent::Function { params, body } => {
                ...
                for (param, param_type) in params.iter().zip(&param_types) {
                    type_env.add_binding(param.clone(), xs_compiler::TypeScheme::mono(param_type.clone()));
                }
                
                // Type check body
                let body_type = type_checker.check(body, &mut type_env)
...
mod tests {
    ...
    fn test_invalidation() {
        ...
        let content_a = DefinitionContent::Value(
            Expr::Literal(Literal::Int(42), Span::new(0, 2))
        );
        let hash_a = store.add_definition(
            path_a.clone(),
            content_a,
            Type::Int,
            HashSet::new(),
