...
with `bench/check_parity` and `bench/cutover` to keep
host vs selfhost output equivalent.

## Macro: stage-level wallclock

Driver: `scripts/bench_perf.sh` → `pkf run bench-selfhost-perf`.
...
Output: `_build/bench/perf/{summary,stage_summary}.{e2e,in-memory}.tsv`.
