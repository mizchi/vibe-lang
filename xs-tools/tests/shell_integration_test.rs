...
fn test_shell_auto_save_to_repository() {
    ...
    let result2 = shell.evaluate_line("(let double (fn (n) (* n 2)))").unwrap();
    ...
    assert!(db_path.exists(), "Code repository database should be created");
}
...
fn test_dependency_tracking() {
    ...
    shell.evaluate_line("(let quad_base (* double_base 2))").unwrap();
...
fn test_namespace_organization() {
    ...
    shell.evaluate_line("(let Math.add (fn (x y) (+ x y)))").unwrap();
    shell.evaluate_line("(let Math.mul (fn (x y) (* x y)))").unwrap();
    shell.evaluate_line("(let String.length (fn (s) (strLength s)))").unwrap();
...
fn test_search_functionality() {
    ...
    shell.evaluate_line("(let addOne (fn (x) (+ x 1)))").unwrap();
    shell.evaluate_line("(let addTwo (fn (x) (+ x 2)))").unwrap();
    shell.evaluate_line("(let multiply (fn (x y) (* x y)))").unwrap();
...
fn test_complex_expressions() {
    ...
    shell.evaluate_line("(rec factorial (n) (if (= n 0) 1 (* n (factorial (- n 1)))))").unwrap();
    ...
    shell.evaluate_line("(let map (fn (f xs) (if (null? xs) (list) (cons (f (car xs)) (map f (cdr xs))))))").unwrap();
    
    // Use with lambda
    let result = shell.evaluate_line("(map (fn (x) (* x 2)) (list 1 2 3))").unwrap();
