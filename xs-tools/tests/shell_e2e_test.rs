...
fn run_test_case(shell: &mut ShellState, test: &TestCase) -> Result<String, String> {
    shell.evaluate_line(test.input)
    ...
    for test in basic_tests() {
        ...
        if test.should_fail {
            ...
            assert!(
                ...
                test.name, test.expected_output, output
            );
        }
    }
}
    ...
    for test in error_tests() {
        ...
        if !test.expected_output.is_empty() {
            ...
            assert!(
                ...
                test.name, test.expected_output, error
            );
        }
    }
}
...
fn test_shell_e2e_repository_tracking() {
    ...
    shell.evaluate_line("(let myAdd (fn (x y) (+ x y)))").unwrap();
    shell.evaluate_line("(let myDouble (fn (x) (* x 2)))").unwrap();
    shell.evaluate_line("(let myQuadruple (fn (x) (myDouble (myDouble x))))").unwrap();
