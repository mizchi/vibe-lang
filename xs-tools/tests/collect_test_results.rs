...
fn generate_test_code(results: &[TestFileResult]) -> String {
    ...
    // Group by directory
    let mut by_dir: std::collections::HashMap<String, Vec<&TestFileResult>> = std::collections::HashMap::new();
    
    for result in results {
        let dir = result.file_path.parent()
    ...
    for (dir, results) in by_dir {
        ...
        code.push_str("    let mut shell = ShellState::new(temp_dir.path().to_path_buf()).unwrap();\n\n");
        ...
        for result in results {
            ...
            if result.is_error_test {
                code.push_str(&format!("    {{\n"));
                code.push_str(&format!("        let result = shell.evaluate_line({:?});\n", result.content));
                ...
                if let Err(ref err) = result.result {
                    ...
                    if !error_key.is_empty() {
                        code.push_str(&format!("        assert!(result.unwrap_err().contains({:?}));\n", error_key));
                    }
                }
                code.push_str("    }\n\n");
            } else {
                code.push_str(&format!("    {{\n"));
                code.push_str(&format!("        let result = shell.evaluate_line({:?});\n", result.content));
                code.push_str(&format!("        assert!(result.is_ok(), \"Test {} failed: {{:?}}\", {:?}, result);\n", 
...
fn collect_and_print_results() {
    ...
    for path in test_files.iter().take(10) { // Start with first 10 files
