...
use xs_compiler::{type_check, TypeChecker, TypeEnv};
use xs_core::parser::parse;
use xs_core::{Expr, Span, Type};
use xs_wasm::wit_generator::WitGenerator;
            ...
            for body_expr in body {
                match body_expr {
                    Expr::Let { name, value, .. } => {
                        ...
                        let scheme = xs_compiler::TypeScheme::mono(typ);
                    ...
                    Expr::Rec {
                        ...
                        type_env.add_binding(
                            name.0.clone(),
                            xs_compiler::TypeScheme::mono(rec_type_var.clone()),
                        );
                        ...
                        for (param, param_type) in params {
                            ...
                            type_env.add_binding(
                                param.0.clone(),
                                xs_compiler::TypeScheme::mono(param_t.clone()),
                            );
                        ...
                        // Store the type
                        let scheme = xs_compiler::TypeScheme::mono(func_type);
                        type_env.add_binding(name.0.clone(), scheme);
                    }
                    Expr::TypeDef { .. } => {
    ...
    for (name, expr) in exports {
        if let Expr::Lambda { .. } = expr {
            ...
            match checker.check(expr, type_env) {
                Ok(typ) => {
                    type_env.add_binding(name.clone(), xs_compiler::TypeScheme::mono(typ.clone()));
                }
    ...
    for (name, expr) in exports {
        match checker.check(expr, type_env) {
            Ok(typ) => {
                ...
                type_env.add_binding(name.clone(), xs_compiler::TypeScheme::mono(typ));
            }
            Err(e) => {
