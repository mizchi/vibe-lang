...
precompile:
    moon build --target native src/cmd/vibe --warn-list '{{moon_warn_list}}'
    mkdir -p dist/std dist/std/path dist/std/threads dist/fs dist/socket dist/http dist/collection dist/encoding dist/json dist/x dist/x/args
    _build/native/debug/build/cmd/vibe/vibe.exe precompile vibe/builtin vibe/builtin/path vibe/builtin/threads vibe/fs vibe/socket vibe/http vibe/collection vibe/encoding vibe/json vibe/x vibe/x/args --out-dir "$(pwd)/dist" --wasm
