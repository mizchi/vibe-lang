...
# Acceptance suite for graduating wasm-gc to the main backend.
# This is intentionally opt-in and currently expected to fail until roadmap work lands.
...
playground-dev: build-integration-deno-wasm
    ...
    cd playground && VITE_VIBE_BUILD_LABEL=$(git rev-parse --short HEAD) pnpm dev

# Build playground for GitHub Pages
playground-build: build-integration-deno-wasm
    cp _build/wasm-gc/release/build/lib/lib.wasm playground/public/vibe-runtime.wasm
    cd playground && VITE_BASE=/vibe-lang/ VITE_VIBE_BUILD_LABEL=$(git rev-parse --short HEAD) pnpm build
