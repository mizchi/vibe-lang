...
| `mappable.vibe` | 2 | Constructor-indexed `Mappable[F[_]]` and pipeline-first `map` for Array and Option |
...
`mappable.vibe` provides the container-preserving transform surface:
...
- `map_opt` remains the explicit Option-qualified helper; generic code uses
  the constructor-indexed `map` above.
...
- Generic transform: `map` through `Mappable[F[_]]`
