...
|---|---|---|---|
| `wasi-command` | `_start` | `wasi:cli/run` when composed, (none) for a bare core | a bare core module: instantiate, apply grants, call `_start` once, discard the instance. A **composed** one (the derived form an executable or test root takes when it depends on a component, [component-build-convention.md](component-build-convention.md) §6.2): the wrap lifts `_start` to the `wasi:cli/run` export and a component host calls that, since it cannot reach the nested core `_start`. Either way once: a second entry call on the same instance is out of contract (`__heap_ptr` and every `let mut` at module scope are live state). That producer does not exist yet (the convention's §1.3 records the retired builder scripts), so the composed column describes what it must emit, not what ships. |
