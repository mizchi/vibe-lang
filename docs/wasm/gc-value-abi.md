...
| Phase | 越えられるようになる境界 | 主な作業 | 備考 |
|---|---|---|---|
| **A** | **返り値・引数** (非ジェネリックな直接呼び出し) | 関数型を静的型から導出。呼び出し側と定義側で型が一致することの検証 | acceptance の1つ目。ここだけで実用価値が出る |
| **B** | **別名・局所束縛** | 既存の `gc_native_array_locals` の追跡を関数間へ拡張 | A の自然な帰結 |
| **C** | **集約フィールド** | ユーザ構造体を実 wasm-gc struct へ (ADR-0052 の `struct.set` 経路の一般化) | ヒープモデルの変更を含み、最も重い |
| **D** | **クロージャ捕捉** | funcref テーブルの型が現状 arity 別 `(i64...)->i64` のみ。型別に増やすか、捕捉は i64 固定にするか | 表が型ごとに増える点が最大の論点 |
