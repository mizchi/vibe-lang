...
**継続 spine 上の通常代入 (`x = perform Op()`) と compound assignment
(`x += perform Op()`) の RHS** でも fresh let を介して一回だけ評価し、元の代入と
演算子を保持して一回だけ書き戻せる (追記45/48)。`perform Op() + 1` のように
suspension を内包する compound RHS と compound selection input は reject のまま。
**concrete な row に
