...
`vibe eval` は、式を逐次評価しながらローカル状態を積み上げ、最後に `--export` で `.vibe` として確定するためのコマンド。
...
```bash
vibe eval --db tmp1.db 'let base = 10'
vibe eval --db tmp1.db 'let inc = (x: Int) -> Int { x + base }'
vibe eval --db tmp1.db 'let answer = inc(2)'
vibe eval --db tmp1.db 'export { answer }'
vibe eval --db tmp1.db --export main.vibe
```

### 2) 型検証しながら進める

```bash
vibe eval --db tmp1.db --assert 'base:Int' --assert 'inc:(x: Int) -> Int'
vibe eval --db tmp1.db --inspect-scope
```

### 3) テストを関数単位で外出し蓄積

```bash
vibe eval --db tmp1.db --test-for add_base \
