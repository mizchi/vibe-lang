...
([host-runtime-contract.md](host-runtime-contract.md) — the *compiler's own*
...
`docs/wasm/host-runtime-contract.json` already partitions all 59 emitted import
...
already exists, as `docs/wasm/host-runtime-contract.json`'s bands, with a
...
4. every import outside `portableCore` that it does not provide → the band in
   `docs/wasm/host-runtime-contract.json` already decides this, and nothing
   here changes it.
...
| the trap is proven to fire | **landed** — `scripts/host_capability_withhold_test.sh` (`pkf run test-host-capability-withhold`, and in `tests/gates/selftests/run.sh`): the granted run reads the file, the withheld run traps by name *after* instantiating and prints no value, and the same run against a copy of the runner with the branch removed succeeds |
| viberun can withhold | **not done** — it can refuse (do not register the import) but has no trapping stub; `Linker::func_new` returning `Err` is the shape, and it is untested here because nothing in this workspace has built that crate |
