...
| **M2c-3 (surface)** | `for await x in stream { ... }` サーフェス構文。host compiler は iterable の型で eager（Array/`Stream[T]`）と pull（`() -> Option[T]` closure）を出し分ける。**selfhost compiler は `await` マーカー自体を pull シグナルとして扱い**、`for await x in s` を parser で `let __pull_next = s; let mut cont = true; while cont { match __pull_next() { Some(x) => body, None => cont = false } }` へ desugar（`build_pull_for_in`、codegen に型情報不要）。マーカー無し `for x in arr` は従来どおり eager `EForIn`。gate の for-await entry は pull closure（bytes("ABC") を yield）で → 42 | done |
