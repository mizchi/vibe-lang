...
migration for positional consumers. Persistent TypeEnv v8 transports those
...
TDRE8 TypeEnv reuse is enabled by default for the `vibe check` filesystem
...
exact TDRE8A/TDRE8W texts on a fresh selfcompile exceeds the signed 2 GiB guest
...
TDRE8 aliases the exact logical
...
`(path, canonical TypeEnv-v8 transport text)` row in the exact ordered resolved
...
and to TDRE8 lookup/publication, so ambient non-direct cache mutations are
semantically irrelevant and avoid quadratic canonical serialization. TDRE8 does
...
The v8 TypeEnv codec round-trips every current `TypeEnv` variant, including
...
constructor-bound self-reference isolation bumps the global persistent cache
namespace from v21 to v22 and replaces TDRE7 aliases/witnesses with disjoint
TDRE8 namespaces and `TDRE8A` / `TDRE8W` envelopes. The preceding kinded-type
transition was v20 to v21 and TDRE6 to TDRE7. Old entries are never
reinterpreted. A
sidecar is still only an alias to a conservative
TypeEnv commit, not a `CheckedProgram` or lossless typed-IR transport.

A witness is published only after that module's canonical TypeEnv-v8 target and
...
persistent TypeEnv-v8 transport result for the dependency is reported as a
separate `changed`/`unchanged` observation rather than being treated as the
exported interface. Production TDRE8 may reuse the unaffected consumer; the
shadow classifier remains observation-only and does not authorize that reuse.
It does not change trace schema 6, cache namespace v22, TypeEnv-v8/TDRE8
