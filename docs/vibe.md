...
- Runtime gate (current CLI behavior):
  ...
  - `Threads::*` is an experimental source-level API surface.
  - `vibe run`, compiled `vibe test`, compiled `vibe bench`/`bench-file`,
    compiled `shell`/`shell-stdin`, `vibe compile`/`compile-lite`,
    `vibe build`, `vibe precompile`, `vibe profile`, and
    `compile-closure-payload` reject `Threads::*` without
  ...
  - with `--unstable-threads`, `Threads::probe_wat()` and
    ...
    with a nominal `ThreadChannel[T]` handle whose payload type is inferred
    from `send` or an explicit annotation before `recv`, and
  ...
  - `--thread-backend <id>` records the intended experimental backend
    ...
    `--unstable-threads`. `Threads::probe_wat()` and
    `Threads::runtime_hints()` can observe non-`linear-local` backends as
    metadata constants, but stateful `Threads::channel_*` and
    `Threads::spawn`/`wait` lowering rejects them until the corresponding
...
Threads (experimental, typechecked with `{Threads}`; all builtins have
compiled/component lowering today, but `spawn` is still a local
...
- `Threads::channel_new(capacity: Int)` -> `ThreadChannel[T]` (opaque local channel handle; `capacity <= 0` means unbounded)
- `Threads::send(channel: ThreadChannel[T], message: T)` -> `Bool` (`false` when a bounded channel is full)
...
- `vibe/prelude/threads.vibe` は test-safe な pure contract 層を分離:
  ...
  - runtime 呼び出しは `{Threads}` effect を要求する
  - `probe_wat` は compiled constant、`runtime_hints` は codegen に渡された
    backend ID に応じた compiled constant として扱う
  - `channel_new`/`send`/`recv` は同一 Wasm instance 内の local FIFO
    channel として扱う
  - `spawn`/`wait` は同一 Wasm instance 内の local immediate-complete task
    として扱う。preemptive/parallel worker 実行ではない
  - CLI/runtime の既定 backend ID は `linear-local`
  - `--thread-backend component-unsafe-os-threads` は Wasmtime fork の
    Vibe slot ABI へ接続するための選択契約を保持するが、現時点の
    compiled stateful lowering (`channel_*`/`spawn`/`wait`) では未接続
    backend として拒否する。`probe_wat`/`runtime_hints` は metadata
    constant として利用できる
