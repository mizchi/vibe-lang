...
## #1262: RC self-build にはまだ別の潜在バグがある (意味的に空の変更で落ちる)

上の修正のついでに、呼び出し元が 0 になった `emit_f64_const_bits` を削除しようと
した (「壊れた形を表現不能にする」= #1416 の教訓)。削除内容は import 一覧 30 行 +
定義 + `.vpkg` の宣言だけで、**意味的には完全に空**である。

結果:

- bump self-build: **通る**
- RC self-build: **stage1 → stage2 で OOB クラッシュ**
- クリーンツリー + `VIBE_REGEN_MODULE_SOURCE=1` での RC ビルド: 通る
  (= 私の変更が原因であることを A/B で確認済み)
- gc の EFloat 修正だけを入れた RC ビルド: 通る (= 削除だけが原因)

**デッド関数を 1 つ消すだけで RC ビルドのコンパイラがクラッシュする。**
これは #1416 と同じクラス — RC 経路がレイアウト依存で、確保パターンが動くと
別の潜在バグが顔を出す — の 3 例目である。したがって:

- `emit_f64_const_bits` の削除は**この PR では見送る** (静的ロックで代替)
- all-RC self-build は「fixpoint が通る」だけでは健全とは言えない。
  少なくとももう 1 つバグが残っている

### bump 撤去の順序への含意

以前「RC への切り替えの障害は correctness ではなく CI wall」と書いたが、**誤り**
だった。correctness が先に立ちはだかっている。順序は変わらない
(RC=1 に倒す → 焼く → bump を消す) が、最初のステップの前提が違う:

1. all-RC self-build で `VIBE_RC=1 compiler_gate.sh` を**完走**させる
   (現状 §40h で停止 → この PR で解消、その先は未走)
2. 残りの潜在バグ (上記) を潰す
3. そこで初めて CI wall が判断材料になる
