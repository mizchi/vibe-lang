...
`Set` / `ArrayBuilder`, and whole enums(`ModuleRef`, `ParamLabel`, `EffectAtom`)this compiler has never had.`scripts / check_ast_binary_tags.sh`
...
Magic: ASCII`'v''A''S''T'`(`0x760x410x530x54`).Followed by a
varint version number; this document defines * * version1 * *.The
deserializer rejects any other magic or any version it doesn'
recognize.
...
|Name|Encoding|
| - --------------| - ---------|
|`varint`|LEB128 unsigned, max10bytes(i.e.fits u64).|
|`svarint`|Standard signed LEB128(sleb128), max10bytes(i.e.fits i64).Chosen to align with wasm / DWARF conventions.|
...
|`f64`|`varint`lo + `varint`hi: the IEEE - 754binary64 bit pattern split into two UNSIGNED32 - bit halves, low half first.See below.|
|`(A, B)`|`A`then`B`, no tag—the field'declared type says which.Likewise for wider tuples.|
|`span`|`svarint(start)svarint(end)`.Implemented and tested, but * * no node in this AST uses it * *: vibe'AST carries bare byte offsets in named`Int`slots(`EIdent`, `ELet`, `ELetMut`, `ECall`, `EBinOp`, `EDot`)rather than start / end pairs, so those encode as plain`svarint`.Retained because it predates the vibe AST and costs nothing.|
...
on read, is * * lossy * *.Measured2026 - 09 - 11against the committed seed
...
value tested, and each half is `0..2^32 - 1`, so each fits vibe'63 - bit
`Int`—unlike the full64 - bit pattern, which does not(ADR - 0055Blocker - 2).This is the same split`emit_f64_const_lohi`already uses to
put an`f64.const`into the wasm output.

* * The read side needed a primitive that did not exist * *, which is why
`Pat`and`Expr`landed after`TypeExpr`rather than with it.
`Double::from_i64_bits(Int) -> Double`takes the whole pattern in one
`Int`, so it cannot receive any value whose bit pattern exceeds63bits—
which includes every negative double(` - 1.5` is `0xBFF8000000000000`).
`Double::from_i64_bits_lohi(Int, Int) -> Double`(#2651) is the
counterpart, taking the same two halves back; all four trees are
implemented on it.

Both halves are validated as unsigned32 - bit before reconstruction.The
primitive masks each to32bits, so an out - of - range half from a corrupt
cache would silently rebuild a * different * literal instead of missing the
cache.
...
A new variant appended to an enum 'declaration therefore takes the next
free tag, NOT the tag its new declaration position would imply.

Field names below are descriptive; the AST'constructors are positional,
so the * * order * * is what the format fixes.

###TypeExpr tags(5variants)
...
encodings(`varint`, `svarint`, `bool`, `string`, `optstr`, `span`, `header`) with canonical - byte and roundtrip checks, plus a canonical
...
shape; pure additions don'require a version bump(the unknown - tag error already protects older readers).
