...
| 0056 | **Selfhost cutover は runner 層と compiler wasm 層を分離して進める**。2026-06-09 時点で local gates は perf TOTAL compile 1.557× ≤2.5、check 0.115× ≤1.33、peak RSS compile 1.398× / check 0.933× ≤2.0、corpus REAL=0、`release-selfhost-gates` green。以後は `tools/moonrun_wasmtime` / cwasm cache / wasmtime flags を実行基盤、`vibe/compiler/` dist/component/check entry を canonical compiler artifact として扱い、MoonBit `src/` は bootstrap/fallback へ縮退する。現在の green 状態を annotated tag + seed manifest で固定し、Rust-style の stage0(seed) -> stage1 -> stage2 bootstrap bump を明示 gate とする。新 syntax を compiler source が使い始めるのは、その syntax を理解する seed を先に tag してから。詳細は [selfhost bootstrap policy](selfhost-bootstrap.md)。削除は selfhost wasm 経路が CI で継続 green になってから段階的に行う。 | accepted |
