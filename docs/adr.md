...
| 0052 | **struct field の `mut` 修飾子**。`struct S { mut field: T }` を許容。frontend desugar で `field__cell: Array[T]` (長さ 1) への純 sugar として lowering。新たな escape 分析は導入せず、既存の `Array[T]` field 経由 mutation と等価のセマンティクス。 | proposed |
