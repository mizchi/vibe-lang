...
| `pure-data` | `vibe/std/array.vibe`, `vibe/std/option.vibe`, `vibe/std/result.vibe`, `vibe/std/bytes.vibe`, `vibe/std/threads/spec.vibe` | Pure data structures and transforms |
...
Collection-focused modules (`list/map/set`) are now managed in `vibe/collection`, outside `vibe/std` layering checks.
