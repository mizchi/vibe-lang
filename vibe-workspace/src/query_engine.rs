...
use crate::code_query::{
    AstNodeType, AstPattern, CodeQuery, PatternType, SearchResult, TypePattern,
};
use crate::hash::DefinitionHash;
use crate::namespace::{
    Definition, DefinitionContent, DefinitionPath, NamespacePath, NamespaceStore,
};
...
impl QueryEngine {
    ...
    fn matches_query(
        ...
        match query {
            ...
            CodeQuery::AstPattern(pattern) => Ok(self.matches_ast_pattern(&def.content, pattern)),

            CodeQuery::DependsOn { target, transitive } => {
                if *transitive {
                    Ok(self.has_transitive_dependency(hash, target))
                } else if let Some(target_hash) =
                    self.namespace_store.get_definition_by_path(target)
                {
            ...
            CodeQuery::NamePattern(pattern) => Ok(self.matches_name_pattern(&path.name, pattern)),

            CodeQuery::InNamespace(ns_path) => Ok(self.is_in_namespace(path, ns_path)),

            CodeQuery::And(q1, q2) => Ok(self.matches_query(path, hash, def, q1)?
                && self.matches_query(path, hash, def, q2)?),

            CodeQuery::Or(q1, q2) => Ok(self.matches_query(path, hash, def, q1)?
                || self.matches_query(path, hash, def, q2)?),

            CodeQuery::Not(q) => Ok(!self.matches_query(path, hash, def, q)?),
        }
    }
    ...
    fn matches_type_pattern(&self, ty: &Type, pattern: &TypePattern) -> bool {
        match pattern {
            ...
            TypePattern::ContainsVar(var_name) => self.type_contains_var(ty, var_name),
        }
    }
    ...
    fn type_contains_var(&self, ty: &Type, var_name: &str) -> bool {
        match ty {
            ...
            Type::Function(in_ty, out_ty) => {
                self.type_contains_var(in_ty, var_name) || self.type_contains_var(out_ty, var_name)
            }
    ...
    fn matches_ast_pattern(&self, content: &DefinitionContent, pattern: &AstPattern) -> bool {
        match pattern {
            AstPattern::Contains(node_type) => match content {
                DefinitionContent::Function { body, .. } => {
                    self.expr_contains_node_type(body, node_type)
                }
                DefinitionContent::Value(expr) => self.expr_contains_node_type(expr, node_type),
                _ => false,
            },

            AstPattern::FunctionWith {
                param_count,
                contains,
                recursive,
            } => {
            ...
            AstPattern::UsesBuiltin(name) => match content {
                DefinitionContent::Function { body, .. } => self.expr_uses_builtin(body, name),
                DefinitionContent::Value(expr) => self.expr_uses_builtin(expr, name),
                _ => false,
            },

            AstPattern::HasPatternMatch {
                min_cases,
                pattern_type,
            } => match content {
                DefinitionContent::Function { body, .. } => {
                ...
                match expr {
                    ...
                    Expr::Apply { func, args, .. } => {
                        self.expr_contains_node_type(func, node_type)
                            || args
                                .iter()
                                .any(|arg| self.expr_contains_node_type(arg, node_type))
                    }
                    ...
                    Expr::Match { expr, cases, .. } => {
                        self.expr_contains_node_type(expr, node_type)
                            || cases
                                .iter()
                                .any(|(_pat, e)| self.expr_contains_node_type(e, node_type))
                    }
                    Expr::List(items, _) => items
                        .iter()
                        .any(|e| self.expr_contains_node_type(e, node_type)),
                    _ => false,
                }
            }
        }
        ...
        match expr {
            ...
            Expr::Apply { func, args, .. } => {
                self.expr_uses_builtin(func, builtin_name)
                    || args
                        .iter()
                        .any(|arg| self.expr_uses_builtin(arg, builtin_name))
            }
            Expr::Let { value, .. } => self.expr_uses_builtin(value, builtin_name),
            Expr::LetIn { value, body, .. } => {
            ...
            Expr::Match { expr, cases, .. } => {
                self.expr_uses_builtin(expr, builtin_name)
                    || cases
                        .iter()
                        .any(|(_, e)| self.expr_uses_builtin(e, builtin_name))
            }
            Expr::List(items, _) => items
                .iter()
                .any(|e| self.expr_uses_builtin(e, builtin_name)),
            _ => false,
        }
    }
    ...
    fn expr_has_pattern_match(
        ...
        match expr {
            Expr::Match { cases, .. } => {
                let case_count_match = min_cases.map(|min| cases.len() >= min).unwrap_or(true);

                let pattern_type_match = pattern_type
                    .as_ref()
            ...
            Expr::Apply { func, args, .. } => {
                self.expr_has_pattern_match(func, min_cases, pattern_type)
                    || args
                        .iter()
                        .any(|arg| self.expr_has_pattern_match(arg, min_cases, pattern_type))
            }
            Expr::Let { value, .. } => self.expr_has_pattern_match(value, min_cases, pattern_type),
            Expr::LetIn { value, body, .. } => {
    ...
    fn get_match_reason(
        ...
        match query {
            CodeQuery::TypePattern(_) => Ok(format!(
                "Type matches: {}",
                self.type_to_string(&def.type_signature)
            )),
            CodeQuery::AstPattern(pattern) => Ok(format!("AST pattern: {pattern:?}")),
            ...
            CodeQuery::NamePattern(pattern) => Ok(format!("Name matches pattern: {pattern}")),
            CodeQuery::InNamespace(ns) => Ok(format!("In namespace: {}", ns.to_string())),
