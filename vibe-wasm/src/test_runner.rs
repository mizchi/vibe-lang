//! Vibe Language test runner framework
//!
...
use vibe_compiler::wasm::generate_module;
use crate::runner::{RunResult, WasmTestRunner};
use std::fs;
use std::path::{Path, PathBuf};
use wasmtime::Val;
use vibe_compiler::transform_to_ir;
        ...
        match (&test.expected, &result) {
            ...
            (Some(TestExpectation::Error(expected)), RunResult::Error(actual)) => {
                if actual.contains(expected) {
                    ...
                    TestResult::Fail(format!("Expected error '{expected}', got '{actual}'"))
                }
            }
            _ => TestResult::Fail(format!(
                "Unexpected result. Expected {:?}, got {:?}",
