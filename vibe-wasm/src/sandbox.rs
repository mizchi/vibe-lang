...
use crate::manifest::PermissionManifest;
use crate::permissions::Permission;
use crate::wasi_error::{PermissionError, Result};
...
impl SandboxedWasiRuntime {
    pub fn new(manifest: PermissionManifest) -> Result<Self> {
        ...
        let engine = Engine::new(&config)
            .map_err(|e| PermissionError::ManifestError(format!("Failed to create engine: {e}")))?;
    ...
    fn configure_filesystem(&self, _builder: &mut WasiCtxBuilder) {
            ...
            match perm {
                Permission::FileRead(pattern) | Permission::FileWrite(pattern) => {
                    if let Ok(dir_path) = self.pattern_to_dir_path(&pattern.pattern) {
                        ...
                        // TODO: In newer wasmtime-wasi versions, directory handling has changed
                        // For now, we'll skip directory preopen
                        // builder.preopened_dir(dir_path, dir_path);
