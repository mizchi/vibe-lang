...
- [ ] vibe-language は wasm-pack でコンパイルできるような純粋なロジックとして、fs を mock すれば動くとする
...
- ✅ 実験的GLLパーサーをメインパーサーに昇格
- ✅ レガシーパーサー（parser_impl.rs、simple_parser.rs）の削除
- ✅ SPPF→AST変換の実装
  - ✅ リテラル値（数値、文字列、真偽値）の変換
  - ✅ let binding構文の変換
  - ✅ 二項演算子の変換（全演算子対応）
  - ✅ ラムダ式の変換（fn x -> body形式）
  - ✅ 関数適用の変換
...
- [x] 実験的GLLパーサーへの移行 ✅
  - 実装済み: レガシーパーサーを削除し、GLLパーサーをメインに
  - SPPF→AST変換の基本実装完了
- [ ] SPPF→AST変換の未実装機能
  - [ ] リスト構文の変換（`[1, 2, 3]`）
  - [ ] if式の変換
  - [ ] パターンマッチング（match式）の変換
  - [ ] 複数式を持つプログラムの処理
  - [ ] import/export文の変換
  - [ ] 型注釈の変換
  - [ ] effect構文の変換
...
### パーサー移行
- [x] レガシーパーサー（hand-written parser）を完全に削除
- [x] 実験的GLLパーサーをメインパーサーに昇格
- [x] パーサー選択機構（ParserSelector）を削除

### SPPF→AST変換
- [x] 基本的なリテラル値の変換（整数、浮動小数点、文字列、真偽値）
- [x] let binding の変換（複数パラメータ対応）
- [x] 二項演算子の変換（全優先順位レベル対応）
- [x] 関数定義（fn x -> body、fn x { body }）の変換
- [x] 関数適用の変換
- [x] ブロック構文の変換
- [x] if式の変換
- [x] match式（パターンマッチング）の変換
- [x] 括弧付き式の変換
- [x] コメント処理（トークンレベル）

### vibe test コマンド
- [x] 新パーサーでのテスト実行が可能に
- [x] 基本的なテストケース（test_simple_42.vibe）がパス

## 既知の問題

### GLLパーサー関連
- [ ] 複数の文を持つプログラムが正しくパースされない
  - 例: `let x = 42` の後の `print x` がパースされない
  - Program ノードの再帰構造がSPPFで正しく表現されていない
  - GLLの文法規則の修正が必要
