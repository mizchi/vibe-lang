...
`grow` の署名が語っていないことに注意してください。row は空で、空の row が
約束するのはホストの権限も代数的エフェクトも使わないことです — 引数から
届く値が変わらないことは**約束しません**。引数の `mut` フィールドに書き込む
場合も同じです。row のラベル `Mut` は、この種のミューテーションを示す将来の
opt-in の印のために予約されています: 今は `with Mut`（や `Mut[..]`）は拒否
され、`Mut` という名前の effect を宣言することもできません。
