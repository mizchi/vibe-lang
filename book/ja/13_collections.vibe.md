...
`xs[i]` と `Array::get(xs, i)` は同じ読み取り。範囲外ではどちらも
**トラップする** — 番兵値で答える代わりにプログラムが止まる。`vibe run` /
`vibe test` では、トラップがアクセス地点の操作名・index・length と編集可能な
`path:line` を出す (#2199)。位置はコンパイラが出すマッピング表から来るので、
表が無いところ — lambda の本体の中、`VIBE_RC=0` と wasm-gc のレーン、表を
strip する `vibe build` の成果物 — には位置が無く、そこでは推測せずに位置
なしのフレームを出す。「無いのが普通」の場面には、下の `MutMap::get` のような
`Option` を返す lookup がある。
