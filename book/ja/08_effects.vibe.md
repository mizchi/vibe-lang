...
resume します。そして、それを明示しなければなりません: 腕のどの出口も
`resume(v)` で終わるか、`return` / `throw` で抜けます。値を返すだけの腕 —
`Ask::Value(_q) => 41` — は拒否されます。`41` で関数を続けるのか、そこで
止めるのかが腕のどこにも書かれていないからです。`Unit` を返す操作は
`resume(())` で再開します。
