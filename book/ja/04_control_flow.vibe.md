...
`Array` に対する `for-in` も式で、本体の結果を集めた `Array` に評価されます。
要素の前に名前を足すと添字が取れます。同じ位置に pull イテレータを置くと、
別のコレクションではなくエラーになります。その場合は
...
副作用のために書いた `for` も同じ式ですが、その値が捨てられるときは配列を
作りません。後ろに文が続く `for`、`while` 本体の最後の文、それ自体が文である
`if` / `match` の分岐がこれに当たります。そうしたループのコストは、同じ要素を
回す `while` と同じです。値を束縛すると配列は残ります: `let _ = for ...` は
配列を作ります。
...
パイプのスロット、次が `(v) -> v * 2` です。見分け方は構文だけで決まり、
これが規則のすべてです: `|>` の直後の呼び出しの引数そのものである裸の `_`
はスロット、演算子のオペランドである `_` はセクション。それ以外の場所の
裸の `_`（パイプの外の `f(_)` など）はエラーになります。
