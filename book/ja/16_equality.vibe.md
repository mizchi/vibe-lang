...
比較子がまだ無いのは二つで、外側の関数の型パラメータを引数にしたものと、
`Nest[T]` が `Nest[Array[T]]` を持つような閉じない再帰である。この二つは
コンパイルでき、比較を実行すると trap する。下のビルドエラーではない。
...
witness が無い呼び出しについては、以上が全部で、コンパイルエラーになる。
構造的な比較子が無い要素は、直し方を名指しするビルドエラーになる。どちらも
アドレスでは答えない。上の、外側の型パラメータと閉じない再帰は例外で、
コンパイルは通り、比較を実行すると trap する。
