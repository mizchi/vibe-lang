...
型引数を取る struct が「自分で型を語る」のは、リテラルに書かれた型引数が
すべて scalar のときだけ。`Box[Int]::{ value: 1 }` は解決するが、
`Box[Array[Int]]::{ value: [1, 2] }` は解決しない。`Box::equals` は struct
全体に対して 1 つだけ生成され、`T` 由来のフィールドを `T` が何だったか
知らないまま比較する — `Int` なら内容比較、配列ならアドレス比較になる。
後者は注釈を付けても直らない。別途修正中の既知の不具合である。
