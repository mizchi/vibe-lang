...
そのメッセージは `Stdout` を名指しします。これは `println` が使う出力の
古いラベルで、直し方として `allows Stdout` と書かせます。書くのは
`allows Console` です。`Stdout` はカバーされますが、`Stdout` だけでは
後から書く `Console::` の操作はカバーされません。
