...
fn test_int_namespace() {
    ...
    let (stdout, stderr, success) = run_xsc(&["exec", "test_int_tostring.xs"]);
    ...
    let (stdout, stderr, success) = run_xsc(&["exec", "test_int_add.xs"]);
    assert!(success, "Run failed: {stderr}");
    ...
    let (stdout, stderr, success) = run_xsc(&["exec", "test_int_eq.xs"]);
    assert!(success, "Run failed: {stderr}");
...
fn test_string_namespace() {
    ...
    let (stdout, stderr, success) = run_xsc(&["exec", "test_string_concat.xs"]);
    assert!(success, "Run failed: {stderr}");
    ...
    let (stdout, stderr, success) = run_xsc(&["exec", "test_string_fromint.xs"]);
    assert!(success, "Run failed: {stderr}");
...
fn test_list_namespace() {
    ...
    let (stdout, stderr, success) = run_xsc(&["exec", "test_list_cons.xs"]);
    assert!(success, "Run failed: {stderr}");
...
fn test_io_namespace() {
    ...
    let (stdout, stderr, success) = run_xsc(&["exec", "test_io_print.xs"]);
    assert!(success, "Run failed: {stderr}");
...
fn test_mixed_namespace_usage() {
    ...
    let (stdout, stderr, success) = run_xsc(&["exec", "test_mixed_namespace.xs"]);
    assert!(success, "Run failed: {stderr}");
...
fn test_backward_compatibility() {
    ...
    let (stdout, stderr, success) = run_xsc(&["exec", "test_backward_plus.xs"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("30"));
    ...
    // let (stdout, stderr, success) = run_xsc(&["exec", "test_backward_strconcat.xs"]);
    // assert!(success, "Run failed: {stderr}");
    // assert!(stdout.contains("\"Hello, World!\""));
}
