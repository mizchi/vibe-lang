...
pub enum ApiCommand {
    ...
    Update { name: String, expr: String },
    Delete { name: String },
    ...
    Inline { name: String, at: String },
    Extract { expr: String, as_name: String },
    Refactor { target: String, operation: RefactorOperation },
...
pub fn process_api_command(shell_state: &mut crate::ShellState, command: ApiCommand) -> String {
    let response = match command {
            ...
            let defs: Vec<_> = definitions
                ...
                .map(|line| {
                    ...
                    if parts.len() >= 2 {
                        ...
                        let typ = type_and_hash.get(0).map(|s| s.to_string()).unwrap_or_default();
                        let hash = type_and_hash.get(1)
        ...
        ApiCommand::TypeOf { expr } => {
            match shell_state.type_of_expr(&expr) {
                Ok(result) => {
        ...
        ApiCommand::Hover { expr } => {
            match shell_state.show_hover_info(&expr) {
