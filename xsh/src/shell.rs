...
impl ShellState {
    pub fn new(storage_path: PathBuf) -> Result<Self> {
        ...
        if index_path.exists() {
            println!("{}", "Loading index.xbin...".cyan());
            if let Err(e) = shell_state.load_xbin(&index_path) {
                eprintln!("{}: Failed to load index.xbin: {}", "Warning".yellow(), e);
            } else {
                println!("{}", "index.xbin loaded successfully".green());
            }
        }
    ...
    pub fn load_xbin(&mut self, path: &PathBuf) -> Result<()> {
