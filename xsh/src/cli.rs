    ...
    match args.command {
        ...
        Command::Check { path, verbose } => {
            ...
            if path.is_file() {
                ...
                // Directory check - find all .xs and .xbin files
                    ...
                    if path.is_file() {
                        ...
                        match ext {
                            ...
                            Some("xbin") => {
                                ...
                                match check_xbin_file(path, false) {
                                    Ok(_) => {
                                        checked_files += 1;
                                        if verbose {
                                            println!("{}", "OK".green());
            ...
            if errors > 0 {
                ...
                println!("{} All files type check successfully!", "Success:".green().bold());
            }
        }
...
fn check_xbin_file(path: &Path, verbose: bool) -> Result<()> {
    let mut storage = XBinStorage::new(path.to_string_lossy().to_string());
    ...
    for (name, hash) in codebase.names() {
        let term = codebase.get_term(&hash).ok_or_else(|| 
            anyhow::anyhow!("Term not found for name: {}", name))?;
        // Terms in xbin are already type checked, but we re-check them
        match type_check(&term.expr) {
            Ok(actual_ty) => {
                ...
                if format_type(&actual_ty) != format_type(&term.ty) {
                    ...
                    errors.push(format!("{} [{}]: Type mismatch - stored: {}, actual: {}", 
                        name, hash.to_hex(), format_type(&term.ty), format_type(&actual_ty)));
                } else {
                    checked += 1;
                }
            }
