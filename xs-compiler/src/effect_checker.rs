...
use xs_core::{
    Expr, 
    extensible_effects::{ExtensibleEffectRow, EffectInstance, EffectDefinition},
};
use crate::TypeEnv;
...
impl EffectChecker {
    pub fn new() -> Self {
        ...
        for (name, def) in xs_core::extensible_effects::builtin_effects() {
    ...
    fn substitute_effects(&self, effects: &ExtensibleEffectRow) -> ExtensibleEffectRow {
        match effects {
            ...
            ExtensibleEffectRow::Extend(e, rest) => {
                ExtensibleEffectRow::Extend(
                    e.clone(),
                    Box::new(self.substitute_effects(rest))
                )
            }
            ExtensibleEffectRow::Union(left, right) => {
                ExtensibleEffectRow::Union(
    ...
    #[allow(dead_code)]
    fn unify_effects(&mut self, e1: &ExtensibleEffectRow, e2: &ExtensibleEffectRow) -> Result<(), String> {
        ...
        match (&e1, &e2) {
            ...
            (ExtensibleEffectRow::Single(eff1), ExtensibleEffectRow::Single(eff2)) if eff1 == eff2 => Ok(()),
    ...
    pub fn infer_effects(&mut self, expr: &Expr, env: &TypeEnv) -> Result<ExtensibleEffectRow, String> {
        match expr {
            // Pure expressions
    ...
    fn combine_effects(&self, e1: ExtensibleEffectRow, e2: ExtensibleEffectRow) -> ExtensibleEffectRow {
        match (&e1, &e2) {
    ...
    fn union_effects(&self, e1: ExtensibleEffectRow, e2: ExtensibleEffectRow) -> ExtensibleEffectRow {
        match (&e1, &e2) {
    ...
    fn remove_effects(&self, effects: ExtensibleEffectRow, to_remove: HashSet<String>) -> ExtensibleEffectRow {
...
mod tests {
    ...
    fn test_perform_effect() {
        ...
        let expr = Expr::Perform {
            ...
            args: vec![Expr::Literal(Literal::String("Hello".to_string()), Span::new(0, 5))],
            span: Span::new(0, 10),
        };
        ...
        assert!(effect_vec.iter().any(|e| e == &EffectInstance::new("IO".to_string())));
    }
    ...
    fn test_handle_partial_removal() {
        ...
        assert_eq!(effect_vec.len(), 1);
        assert!(effect_vec.iter().any(|e| e == &EffectInstance::new("IO".to_string())));
        assert!(!effect_vec.iter().any(|e| e == &EffectInstance::new("State".to_string())));
    }
}