...
use crate::text_document::TextDocuments;
...
impl XSLanguageServer {
    ...
    async fn analyze_document(&self, uri: &Url) -> Result<()> {
        ...
        // Parse the document
        match xs_core::parser::parse(&content) {
            Ok(expr) => {
                // Generate source map
                ...
                // Type check
                match xs_compiler::type_check(&expr) {
                    Ok(_type) => {
    ...
    fn parse_error_to_diagnostic(&self, error: xs_core::XsError, _uri: &Url) -> Diagnostic {
        let (position, message) = match error {
            xs_core::XsError::ParseError(pos, msg) => (pos, msg),
            _ => (0, format!("{}", error)),
        };
    ...
    fn type_error_to_diagnostic(&self, error: xs_core::XsError, _uri: &Url) -> Diagnostic {
        let (span, message) = match error {
            xs_core::XsError::TypeError(span, msg) => (span, msg),
            _ => (xs_core::Span::new(0, 0), format!("{}", error)),
        };
...
impl LanguageServer for XSLanguageServer {
    ...
    async fn goto_definition(&self, params: GotoDefinitionParams) -> Result<Option<GotoDefinitionResponse>> {
        crate::handlers::goto_definition::handle_goto_definition(self, params).await
    }

    async fn references(&self, params: ReferenceParams) -> Result<Option<Vec<Location>>> {
        crate::handlers::references::handle_references(self, params).await
    }
