...
const SCRIPTS = [
  "selfcompile_heap_policy_canonical_json.mjs",
...
export function validateDockerAuthorityEnvironment(env) {
  if (env.DOCKER_HOST || env.DOCKER_CONTEXT) fail("remote-docker-forbidden");
}

export function parseDockerContextEndpoint(output) {
  const text = String(output);
  const serialized = text.endsWith("\r\n") ? text.slice(0, -2) : text.endsWith("\n") ? text.slice(0, -1) : text;
  if (serialized === "" || /[\r\n]/.test(serialized)) fail("remote-docker-forbidden");
  let endpoint;
  try { endpoint = JSON.parse(serialized); } catch { fail("remote-docker-forbidden"); }
  if (typeof endpoint !== "string" || JSON.stringify(endpoint) !== serialized || !endpoint.startsWith("unix:///")) fail("remote-docker-forbidden");
  let parsed;
  try { parsed = new URL(endpoint); } catch { fail("remote-docker-forbidden"); }
  if (parsed.protocol !== "unix:" || parsed.host !== "" || parsed.username !== "" || parsed.password !== "" || parsed.search !== "" || parsed.hash !== "" || !parsed.pathname.startsWith("/") || parsed.pathname.length < 2) {
    fail("remote-docker-forbidden");
  }
  return endpoint;
}

export function inspectDockerAuthority(runDocker = docker, env = process.env) {
  validateDockerAuthorityEnvironment(env);
  const context = runDocker(["context", "show"]).stdout.trim();
  if (context === "" || /[\r\n]/.test(context)) fail("remote-docker-forbidden");
  const inspected = runDocker(["context", "inspect", context, "--format", "{{json .Endpoints.docker.Host}}"]).stdout;
  return { context, endpoint: parseDockerContextEndpoint(inspected) };
}
...
function verifyImage(lock) {
  const authority = inspectDockerAuthority();
...
export function verifyRecord(output, key, expected) {
  ...
  if (Buffer.from(canonicalJson(record)).toString("base64url") !== parts[0]) fail("noncanonical-container-result");
  ...
  const validHostile = expected.hostileFixtures
    ? hostile !== null && typeof hostile === "object" && !Array.isArray(hostile) && Object.keys(hostile).sort().join(",") === "denied,fake_prefix_captured,markers_absent,positive_stat_calls,safe,schema,timed_out" && hostile.schema === 1 && safePositive(hostile.denied) && hostile.denied >= 12 && safePositive(hostile.safe) && hostile.safe >= 2 && hostile.timed_out === 1 && hostile.fake_prefix_captured === 1 && hostile.markers_absent === true && safePositive(hostile.positive_stat_calls)
    : hostile === null;
  if (keys !== "canonical_root,heap_bytes,hostile_fixture_attestation,label,oid,output_sha256,schema,stage2_sha256,stat_token_attestations,tree,trials" || record.schema !== 1 || record.label !== expected.label || record.oid !== expected.oid || record.tree !== expected.tree || record.canonical_root !== "/workspace/repo" || !validHostile || !safePositive(record.heap_bytes) || !Array.isArray(record.trials) || record.trials.length !== 2 || record.trials.some(value => !safePositive(value)) || record.trials[0] !== record.trials[1] || record.heap_bytes !== record.trials[0] || !/^[0-9a-f]{64}$/.test(record.stage2_sha256) || !Array.isArray(record.output_sha256) || record.output_sha256.length !== 2 || record.output_sha256.some(hash => !/^[0-9a-f]{64}$/.test(hash)) || record.output_sha256[0] !== record.output_sha256[1] || !Array.isArray(record.stat_token_attestations) || record.stat_token_attestations.length !== 2 || !record.stat_token_attestations.every(validAttestation) || JSON.stringify(record.stat_token_attestations[0]) !== JSON.stringify(record.stat_token_attestations[1])) fail("invalid-container-result");
  return {
...
export function measureTreeInDocker({ trustedRoot, lease, archivePath, inputBlob, input, label, oid, tree, seedSha }) {
  ...
  const dockerAuthority = verifyImage(lock);
  ...
  try {
    ...
    return { ...verifyRecord(started.stdout, key, { label, oid, tree, input, hostileFixtures }), docker_authority: dockerAuthority };
  } finally {
