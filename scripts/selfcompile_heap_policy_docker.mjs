...
export function verifyRecord(output, key, expected) {
  ...
  const validHostile = expected.hostileFixtures
    ? hostile !== null && typeof hostile === "object" && !Array.isArray(hostile) && Object.keys(hostile).sort().join(",") === "denied,fake_prefix_captured,markers_absent,positive_stat_calls,safe,schema,timed_out" && hostile.schema === 1 && safePositive(hostile.denied) && hostile.denied >= 11 && safePositive(hostile.safe) && hostile.safe >= 2 && hostile.timed_out === 1 && hostile.fake_prefix_captured === 1 && hostile.markers_absent === true && safePositive(hostile.positive_stat_calls)
    : hostile === null;
