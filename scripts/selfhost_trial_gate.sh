...
if [ "${1:-}" != "--post-generation" ]; then
  echo "== selfhost trial: stage generation =="
  bash scripts/selfhost_generations.sh build --stage3
fi

echo "== selfhost trial: release selfhost gates =="
pkf run release-selfhost-gates

echo "== selfhost trial: corpus REAL-gap gate =="
