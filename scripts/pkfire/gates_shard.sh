...
case "$shard" in
  ...
  cli)
    ...
    #   test_selfhost_cli_direct_parity.sh, test_cutover_gate.sh
  ...
  check)
    ...
    VIBE_CHECK_PARITY_REQUIRE_PARITY=1 ./scripts/test_check_parity.sh
  ...
  coverage)
    ...
    VIBE_SELFHOST_PERF_CASES_FILE=bench/perf/kpi_cases.txt \
