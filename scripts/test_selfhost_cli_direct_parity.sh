...
if [ -z "$host_import_result" ]; then
  echo "selfhost cli direct parity gate failed: host import result missing" >&2
  exit 1
fi
if [ "$selfhost_import_result" != "42" ]; then
