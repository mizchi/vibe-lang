...
# 2. `vibe run --break prog.vibe:1` pauses at the line-1 function (helper),
#    prints `breakpoint hit: prog.vibe:1`, the call stack, then continues.
broke="$(VIBE_BREAK_AUTO=1 "$VIBE" run --break "prog.vibe:1" "$proj/prog.vibe" 2>&1)"
echo "----- vibe run --break prog.vibe:1 -----"; printf '%s\n' "$broke"; echo "----------------------------------------"
printf '%s\n' "$broke" | grep -qF "breakpoint hit: prog.vibe:1" && ok "line breakpoint hit prints prog.vibe:1" || bad "missing 'breakpoint hit: prog.vibe:1'"
...
bare="$(VIBE_BREAK_AUTO=1 "$VIBE" run --break "1" "$proj/prog.vibe" 2>&1)"
printf '%s\n' "$bare" | grep -qF "breakpoint hit: prog.vibe:1" && ok "bare line spec hits prog.vibe:1" || bad "bare line spec did not hit line 1 (got: $bare)"

# 4. A non-matching line spec does NOT pause but still runs to completion.
nohit="$(VIBE_BREAK_AUTO=1 "$VIBE" run --break "prog.vibe:99" "$proj/prog.vibe" 2>&1)"
...
fn="$(VIBE_BREAK_AUTO=1 "$VIBE" run --break helper "$proj/prog.vibe" 2>&1)"
