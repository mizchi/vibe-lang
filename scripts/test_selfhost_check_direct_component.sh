...
bash "$SCRIPT_DIR/run_selfhost_check_direct_component.sh" "$COMPONENT_PATH" "$OK_INPUT" "$OK_OUTPUT"
set +e
bash "$SCRIPT_DIR/run_selfhost_check_direct_component.sh" "$COMPONENT_PATH" "$ERR_INPUT" "$ERR_OUTPUT"
ERR_STATUS=$?
set -e
