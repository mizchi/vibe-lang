...
#
# ## Numbering has a gap, on purpose
#
# RED 3, 5 and 6 are retired, and the survivors keep their original numbers so
# the history each one carries (Codex round 4 on #2753 for the begins-with
# distinction, say) still resolves. #2778 retired the gate's #2737 dispatch
# family -- those shapes compile now -- so:
#
#   RED 3  the edit-clause assertion binds        -> RED 8 proves it, on the
#                                                    erased family's own regex
#   RED 5  the leads-check is begins-with, not    -> RED 9 proves it, through the
#          contains                                  same shared awk
#   RED 6  the reason is selected PER FAMILY      -> there is one family; the
#                                                    property no longer exists
#
# So two of the three lost nothing: RED 8 and RED 9 were already the erased
# family's copies of exactly those properties, written when that family joined.
# RED 6 is a real subtraction, and RED 7 is what covers the ground it leaves --
# a fixture in no family FAILS, so a second family cannot be added unnamed.
set -euo pipefail
...
if ! run_gate "fixtures/lambda_bound_erased_interp_refused.vibe"; then
  cat "$WORK/out" >&2
  fail "the gate does not pass on the unmutated corpus; the red cases below would prove nothing"
fi

# RED 1: the program compiles. The body is the erased-interp fixture with the
# formal given the method-bearing bound the message asks for -- the very edit it
# names, so this case is also the proof that the named edit works. Written out
# rather than derived by sed: the mutation that matters is structural, and an
# edit that lands as a syntax error would "fail" the gate for the wrong reason
# while proving nothing (it did, the first time this was written).
mkdir -p "$WORK/r1"
cat > "$WORK/r1/lambda_bound_erased_interp_bounded_refused.vibe" <<'VIBE'
trait Show {
...
export fn _start() -> Int {
  String::length(render(Pt::{ v: 1 }))
}
VIBE
if run_gate "$WORK/r1/*.vibe"; then
  fail "RED 1: the gate passed a fixture that COMPILES (the refusal is not being checked)"
fi
# The verdict must be "it compiled", which is only reachable if the bounded
# program really does compile -- so this assertion doubles as the
# mutation-landed check.
grep -qF 'compiled; expected a compile-time refusal' "$WORK/out" \
  || { cat "$WORK/out" >&2; fail "RED 1 failed for the wrong reason (the bounded program did not compile)"; }

# RED 2: refused, but for an unrelated reason. "did not compile" must not pass.
mkdir -p "$WORK/r2"
cat > "$WORK/r2/lambda_bound_erased_interp_unrelated_refused.vibe" <<'VIBE'
export fn _start() -> Int {
...
mkdir -p "$WORK/r7"
cp fixtures/lambda_bound_erased_interp_refused.vibe \
...
echo "[lambda-bound-refusal-test] ok (6 red cases, each mutation verified to land)"
