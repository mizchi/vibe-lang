...
for bad in bad_let bad_assign bad_if bad_struct bad_locallet bad_missingfield bad_fnannot bad_return bad_genhead bad_builtinarg bad_dupfield bad_some2 bad_optfield; do
  VIBE_PREOPEN_DIR="$ROOT_DIR" VIBE_FS_COMPILE=1 VIBE_SELFHOST_IMPORT_ABI=raw \
    bash scripts/run_wasm_vibe_host_runner.sh --invoke cli_main "$stage2_wasm" \
...
for bad in bad_match bad_array bad_call bad_calloption bad_not bad_forint bad_tuparity; do
  VIBE_PREOPEN_DIR="$ROOT_DIR" VIBE_FS_COMPILE=1 VIBE_SELFHOST_IMPORT_ABI=raw \
    bash scripts/run_wasm_vibe_host_runner.sh --invoke cli_main "$stage2_wasm" \
...
for bad in bad_index bad_tuple bad_idxtype bad_idxelem bad_arrget bad_arrpush bad_arrset bad_guard bad_arrmap bad_arrfold bad_arrslice; do
  VIBE_PREOPEN_DIR="$ROOT_DIR" VIBE_FS_COMPILE=1 VIBE_SELFHOST_IMPORT_ABI=raw \
    bash scripts/run_wasm_vibe_host_runner.sh --invoke cli_main "$stage2_wasm" \
