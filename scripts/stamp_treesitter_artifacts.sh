...
# Build into a temporary file and rename it over the stamp only once every
# artifact has been found and hashed. Writing straight to `$STAMP` truncated it
# the instant the redirection opened, so a failure inside the loop destroyed a
# valid manifest and left a five-line header behind -- measured with parser.c
# absent, which the corpus check does not look at, so the run got that far
# (#2422 review). A partial write is the one outcome a stamp must never have.
STAMP_TMP="$STAMP.tmp.$$"
