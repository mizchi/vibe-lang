...
#
# usage:
#   scripts/profile_selfhost_compile.sh <stage2.wasm> [input.vibe] [top_n]
#
#   <stage2.wasm>  a selfhost compiler artifact (e.g. the output of
#                  `scripts/selfhost_generations.sh build`)
...
if [ -z "$STAGE2" ] || [ ! -f "$STAGE2" ]; then
  echo "usage: scripts/profile_selfhost_compile.sh <stage2.wasm> [input.vibe] [top_n]" >&2
  exit 2
fi
if [ ! -f "$INPUT" ]; then
