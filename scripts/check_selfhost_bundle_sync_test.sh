...
if ! grep -Fq "$TMP_ROOT/vibe/compiler/selfhost_sources_bundle.vibe" "$TMP_ROOT/main.stderr"; then
...
if ! grep -Fq "$TMP_ROOT/vibe/compiler/selfhost_cli_adapter_bundle.vibe" "$TMP_ROOT/adapter.stderr"; then
