...
test("readAttemptsTsv + buildAggregatedReport + buildMarkdown", () => {
  ...
  fs.writeFileSync(
    attemptsPath,
    [
      `vibe/prelude/a_test.vibe\twasm\t0\t${reportAPath}.log\t${reportAPath}`,
      `vibe/prelude/b_test.vibe\twasm\t0\t${reportBPath}.log\t${reportBPath}`,
      `vibe/prelude/c_test.vibe\twasm\t1\t${logC1}\t-`,
      `vibe/prelude/c_test.vibe\twasm-js-string\t1\t${logC2}\t-`,
    ].join("\n") + "\n",
  );
  ...
  const matrix = {
    ...
    cases: {
      "vibe/prelude/c_test.vibe": { expected_backend: "wasm-js-string" },
    },
  };

  const report = buildAggregatedReport({
    reportPaths: [reportAPath, reportBPath],
    failedCases: ["vibe/prelude/c_test.vibe"],
    attempts,
    totalCases: 3,
  ...
  assert.match(markdown, /^# vibe\/prelude wasm source coverage report/m);
...
test("expectedBackendForCase: defaults and per-case override", () => {
  ...
  const a = expectedBackendForCase(matrix, "vibe/prelude/int_test.vibe");
  assert.equal(a.expected_backend, "wasm");
  assert.deepEqual(a.expected_modes, ["wasm"]);
  assert.equal(a.source, "default");

  const b = expectedBackendForCase(matrix, "vibe/prelude/io_test.vibe");
...
test("loadBackendMatrix: parses json file", () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      defaults: { expected_backend: "wasm" },
      cases: {
        "vibe/prelude/io_test.vibe": { expected_backend: "wasm-js-string" },
      },
    }),
  );
  ...
  assert.equal(
    matrix.cases["vibe/prelude/io_test.vibe"].expected_backend,
    "wasm-js-string",
  );
});
...
test("buildAggregatedReport: classifies expected_failure when expected backend not attempted", () => {
  const attempts = [
    {
      case_path: "vibe/prelude/io_test.vibe",
      mode: "wasm",
      exit_code: 1,
      log_path: "",
      report_path: "-",
  ...
  const report = buildAggregatedReport({
    reportPaths: [],
    failedCases: ["vibe/prelude/io_test.vibe"],
    attempts,
    totalCases: 1,
    backendMatrix: matrix,
  });
...
test("buildAggregatedReport: detects measured mode mismatch", () => {
  ...
  const attempts = [
    {
      case_path: "vibe/prelude/mismatch_test.vibe",
      mode: "wasm-js-string",
      exit_code: 0,
...
test("buildAggregatedReport: ignores excluded lines in line totals", () => {
  ...
  const attempts = [
    {
      case_path: "vibe/prelude/excluded_test.vibe",
      mode: "wasm",
      exit_code: 0,
      log_path: `${reportPath}.log`,
      report_path: reportPath,
