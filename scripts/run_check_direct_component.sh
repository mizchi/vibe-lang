...
if [ "$#" -ne 3 ]; then
  echo "usage: $0 <component.wasm> <input.vibe> <output.txt>" >&2
  exit 1
fi
...
RESULT="$(
  ...
  env VIBE_WASMTIME_WASM_FLAGS="$WASMTIME_WASM_FLAGS" \
    ...
    run \
    --dir . \
    --invoke "run-check-request($INPUT_EXPR,$OUTPUT_EXPR)" \
    "$COMPONENT_PATH"
)"

STATUS="$(printf '%s\n' "$RESULT" | grep -E '^-?[0-9]+$' | tail -n 1 || true)"
if [ -z "$STATUS" ]; then
  echo "component returned no numeric status" >&2
  exit 1
fi
exit "$STATUS"
