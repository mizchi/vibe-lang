...
# #1571: compiled as is -- the expected value is an inspect() test block in
# the fixture, not a `__DATA__` tail to `sed` off into a temp copy.
VIBE_BACKEND=gc VIBE_PREOPEN_DIR="$ROOT_DIR" VIBE_IMPORT_ABI=raw \
  bash scripts/run_wasm_vibe_host_runner.sh --invoke cli_main "$stage2_wasm" \
...
if ! gcclos_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$gcclosdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_call_evidence_closure_literal.vibe under gc tests failed -- hoisted closure row backfill or try-full-before-dropping-dead regressed" >&2
  echo "$gcclos_out" >&2
  exit 1
fi
rm -rf "$gcclosdir"
...
if ! gcidx_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$gcidxdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: gc_backend_effect_pure_builtin_index.vibe under gc tests failed -- pure-builtin allowlist regressed" >&2
  echo "$gcidx_out" >&2
  exit 1
fi
rm -rf "$gcidxdir"
...
if ! gcsuberr_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$gcsuberrdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: gc_backend_suberror_ctor.vibe under gc tests failed -- suberror ctor registration regressed" >&2
  echo "$gcsuberr_out" >&2
  exit 1
fi
rm -rf "$gcsuberrdir"
...
if ! m2_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$m2dir/m2.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_replay_corruption tests failed. This means the fix regressed -- e.g. inline_direct_performs stopped firing for this fixture's shape and it fell back to buggy replay (6016)." >&2
  echo "$m2_out" >&2
  exit 1
fi
rm -rf "$m2dir"
...
if ! a71e_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$a71edir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_operation_level_discharge tests failed" >&2
  echo "$a71e_out" >&2
  exit 1
fi
rm -rf "$a71edir"
...
if ! edp_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edpdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_call_evidence.vibe tests failed -- evidence-dict threading through a helper call regressed" >&2
  echo "$edp_out" >&2
  exit 1
fi
rm -rf "$edpdir"
...
if ! edpb_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edpbdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_call_evidence_branch.vibe tests failed -- either the invalid-wasm crash regressed, or the evidence-dict rewrite for branching bodies regressed" >&2
  echo "$edpb_out" >&2
  exit 1
fi
rm -rf "$edpbdir"
...
if ! edpem_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edpemdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_call_evidence_error_mix.vibe tests failed" >&2
  echo "$edpem_out" >&2
  exit 1
fi
rm -rf "$edpemdir"
...
if ! edpme_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edpmedir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_multi_effect_row_nested.vibe tests failed -- multi-effect nested-handle migration regressed" >&2
  echo "$edpme_out" >&2
  exit 1
fi
rm -rf "$edpmedir"
...
if ! edpnh_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edpnhdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_multi_effect_nested_handles.vibe tests failed -- directly-nested handle pair migration regressed" >&2
  echo "$edpnh_out" >&2
  exit 1
fi
rm -rf "$edpnhdir"
...
if ! edplet_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edpletdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_call_evidence_let_bound.vibe tests failed -- let-bound perform inside a needing function's body regressed (either an uncaught-exception crash or a wrong value)" >&2
  echo "$edplet_out" >&2
  exit 1
fi
rm -rf "$edpletdir"
...
if ! edppur_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edppurdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_call_evidence_pure_helper.vibe tests failed -- pure-helper-call eligibility regressed" >&2
  echo "$edppur_out" >&2
  exit 1
fi
rm -rf "$edppurdir"
...
if ! edpdot_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edpdotdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_call_evidence_struct_field.vibe tests failed -- EDot eligibility regressed" >&2
  echo "$edpdot_out" >&2
  exit 1
fi
rm -rf "$edpdotdir"
...
if ! edpclo_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edpclodir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_call_evidence_closure_literal.vibe tests failed -- closure-literal eligibility regressed" >&2
  echo "$edpclo_out" >&2
  exit 1
fi
rm -rf "$edpclodir"
...
if ! edpes_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edpesdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_call_evidence_effectset_alias.vibe tests failed -- either effectset-alias row recognition regressed, or it regressed back to the replay-inflated value" >&2
  echo "$edpes_out" >&2
  exit 1
fi
rm -rf "$edpesdir"
...
if ! edpqo_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edpqodir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_call_evidence_qualified_op.vibe tests failed -- qualified-operation row recognition regressed, or it regressed back to the replay-inflated value" >&2
  echo "$edpqo_out" >&2
  exit 1
fi
rm -rf "$edpqodir"
...
if ! edprv_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edprvdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_call_evidence_row_variable_tail.vibe tests failed -- row-variable-tail row recognition regressed, or it regressed back to the replay-inflated value" >&2
  echo "$edprv_out" >&2
  exit 1
fi
rm -rf "$edprvdir"
...
if ! edplccf_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edplccfdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_call_evidence_local_closure_capture_free.vibe tests failed -- either #786's hoist regressed or evidence_dict_pass no longer forwards to the hoisted top-level name" >&2
  echo "$edplccf_out" >&2
  exit 1
fi
rm -rf "$edplccfdir"
...
if ! lccc_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$lcccdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_local_closure_capture_conversion.vibe tests failed -- #1069's closure conversion regressed" >&2
  echo "$lccc_out" >&2
  exit 1
fi
rm -rf "$lcccdir"
...
if ! lcwa_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$lcwadir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: local_closure_wrapper_wrong_arity_not_inlined.vibe tests failed -- trivial-wrapper pattern misfired on a wrong-arity call" >&2
  echo "$lcwa_out" >&2
  exit 1
fi
rm -rf "$lcwadir"
...
if ! illha_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$illhadir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_inline_lambda_literal_hof_arg.vibe tests failed -- inline lambda literal IIFE/HOF-arg fix regressed" >&2
  echo "$illha_out" >&2
  exit 1
fi
rm -rf "$illhadir"
...
if ! illla_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$illladir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_inline_lambda_literal_labeled_arg.vibe tests failed -- labeled-arg literal fix regressed" >&2
  echo "$illla_out" >&2
  exit 1
fi
rm -rf "$illladir"
...
if ! dtpws_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$dtpwsdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: dtpw_wrapper_shadowed_by_parameter.vibe tests failed -- trivial-wrapper shadowing fix regressed" >&2
  echo "$dtpws_out" >&2
  exit 1
fi
rm -rf "$dtpwsdir"
...
if ! edps_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edpsdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: evidence_dict_needing_shadowed_by_local.vibe tests failed -- evidence-dict needing-forwarding shadowing fix regressed" >&2
  echo "$edps_out" >&2
  exit 1
fi
rm -rf "$edpsdir"
...
if ! edpg_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edpgdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_local_closure_by_value_hof_general.vibe tests failed -- #1070 general-case closure-HOF-parameter fix regressed" >&2
  echo "$edpg_out" >&2
  exit 1
fi
rm -rf "$edpgdir"
...
if ! edpe_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edpedir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_local_closure_by_value_hof_escaping.vibe tests failed -- #1070 closure-HOF-parameter safety boundary regressed" >&2
  echo "$edpe_out" >&2
  exit 1
fi
rm -rf "$edpedir"
...
if ! edph_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$edphdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_local_closure_handle_owner_param.vibe tests failed -- #1070 self-discharging-owner closure-param fix regressed" >&2
  echo "$edph_out" >&2
  exit 1
fi
rm -rf "$edphdir"
...
if ! g1087_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$g1087dir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_handle_error_nontail tests failed -- a non-tail throw in a with-Error handle body ran the body's continuation instead of aborting to the arm's value (#1087; check idp_arms_discharge_error in inline_direct_perform.vibe)" >&2
  echo "$g1087_out" >&2
  exit 1
fi
rm -rf "$g1087dir"
...
if ! v2_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$v2dir/vacuous.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_vacuous_handle_erased tests failed -- vacuous-handle elimination regressed" >&2
  echo "$v2_out" >&2
  exit 1
fi
...
if ! csib_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$csibdir/out.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: closure_shadowed_inline_builtin tests failed -- either a shadowed inline builtin fell back to the builtin instead of the captured closure (#1114), or a non-recursive let binder was treated as an enclosing shadow inside its own initializer (#1120 Codex P1)" >&2
  echo "$csib_out" >&2
  exit 1
fi
rm -rf "$csibdir"
...
if ! g1340_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$g1340dir/pos.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_generic_row_instantiation tests failed" >&2
  echo "$g1340_out" >&2
  exit 1
fi
...
if ! op_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$opb/pos.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_builtin_operation_row tests failed" >&2
  echo "$op_out" >&2
  exit 1
fi
...
if ! af_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$afdir/pos.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: effect_async_for_row tests failed -- the await lowering of the async for loop regressed" >&2
  echo "$af_out" >&2
  exit 1
fi
...
if ! exc_out="$(VIBE_PREOPEN_DIR="$ROOT_DIR" bash scripts/run_wasm_vibe_host_runner.sh --invoke _start "$excdir/pos.wasm" 2>&1)"; then
  echo "[compiler-gate] FAIL: exception_typed_row tests failed -- kinded exceptions must share one Wasm tag" >&2
  echo "$exc_out" >&2
  exit 1
fi
