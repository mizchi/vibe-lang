...
MATRIX_PATH="${VIBE_WASM_STD_COVERAGE_MATRIX:-$PROJECT_ROOT/vibe/builtin/backend_capabilities.json}"
...
mapfile -t tests < <(find vibe/builtin -name '*_test.vibe' | sort)
if [ -n "$FILTER" ]; then
...
if [ "${#tests[@]}" -eq 0 ]; then
  echo "[wasm std coverage] no test files selected under vibe/builtin" >&2
  exit 1
fi
