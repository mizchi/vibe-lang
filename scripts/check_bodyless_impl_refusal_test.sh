#!/usr/bin/env bash
# Red test for check_bodyless_impl_refusal.sh (#2248's rule: a gate means
...
unset BODYLESS_IMPL_REFUSAL_FIXTURES
unset VIBE_BODYLESS_IMPL_REFUSAL_ROOT
...
# committed seed. The selftests lane shares the compiler-gate matrix precisely
# so its companions can read the lane's compiler out of the environment
# (tests/gates/selftests/run.sh), and `gate_resolve_stage2` exports
# VIBE_STAGE2_WASM for that.
STAGE2_OVERRIDE="${BODYLESS_IMPL_REFUSAL_STAGE2:-${VIBE_STAGE2_WASM:-}}"
unset BODYLESS_IMPL_REFUSAL_STAGE2

WORK="$(mktemp -d "${TMPDIR:-/tmp}/vibe_bodyless_selftest.XXXXXX")"
trap 'rm -rf "$WORK"' EXIT

fail() { echo "[bodyless-impl-refusal-test] FAIL: $*" >&2; exit 1; }

run_gate() { # run_gate <glob>
  if [ -n "$STAGE2_OVERRIDE" ]; then
    BODYLESS_IMPL_REFUSAL_STAGE2="$STAGE2_OVERRIDE" BODYLESS_IMPL_REFUSAL_FIXTURES="$1" \
      bash scripts/check_bodyless_impl_refusal.sh >"$WORK/out" 2>&1
  else
    BODYLESS_IMPL_REFUSAL_FIXTURES="$1" \
      bash scripts/check_bodyless_impl_refusal.sh >"$WORK/out" 2>&1
  fi
}
...
run_mutated_gate() { # run_mutated_gate <gate path> <glob>
  if [ -n "$STAGE2_OVERRIDE" ]; then
    VIBE_BODYLESS_IMPL_REFUSAL_ROOT="$ROOT_DIR" BODYLESS_IMPL_REFUSAL_STAGE2="$STAGE2_OVERRIDE" \
      BODYLESS_IMPL_REFUSAL_FIXTURES="$2" bash "$1" >"$WORK/out" 2>&1
  else
    VIBE_BODYLESS_IMPL_REFUSAL_ROOT="$ROOT_DIR" BODYLESS_IMPL_REFUSAL_FIXTURES="$2" \
      bash "$1" >"$WORK/out" 2>&1
  fi
}
...
if ! run_gate "fixtures/bodyless_impl_*_refused.vibe"; then
  cat "$WORK/out" >&2
  fail "the gate does not pass on the unmutated corpus; the red cases below would prove nothing"
fi

# RED 1: the program COMPILES. The body is the refused fixture with the missing
# method actually GIVEN A BODY -- the very edit the message names, so this case
# is also the proof that the named edit works.
mkdir -p "$WORK/r1"
cat > "$WORK/r1/bodyless_impl_bodied_refused.vibe" <<'VIBE'
trait Greet {
  hello(Self) -> Int
}
...
# The verdict must be "it compiled", which is only reachable if writing the body
# really does make the program compile -- so this doubles as the
...
grep -qF 'compiled; expected a compile-time refusal' "$WORK/out" \
  || { cat "$WORK/out" >&2; fail "RED 1 failed for the wrong reason (adding the body did not make it compile)"; }

# RED 2: refused, but for an unrelated reason. "did not compile" must not pass.
mkdir -p "$WORK/r2"
cat > "$WORK/r2/bodyless_impl_unrelated_refused.vibe" <<'VIBE'
export fn main() -> Int {
...
grep -qF 'refused without the #2872 message' "$WORK/out" \
...
# "Contains" would pass; "begins with" must fail.
mkdir -p "$WORK/r3"
cp fixtures/bodyless_impl_missing_method_refused.vibe \
   "$WORK/r3/bodyless_impl_keep_refused.vibe"
sed 's/^EDIT_NEEDLE="add a body for "$/EDIT_NEEDLE="to fall back to"/' \
  scripts/check_bodyless_impl_refusal.sh > "$WORK/r3/gate.sh"
grep -q '^EDIT_NEEDLE="to fall back to"$' "$WORK/r3/gate.sh" \
...
grep -qF 'to fall back to' "$ROOT_DIR/_build/_bodyless_impl_refusal/bodyless_impl_keep_refused.wasm.diag" 2>/dev/null \
...
echo "[bodyless-impl-refusal-test] ok (4 red cases, each mutation verified to land)"
