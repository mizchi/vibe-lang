...
#
#   scripts/coverage_selfhost_testexec.sh
#
...
[ -s "$CORPUS_ACC" ] || { echo "testexec: corpus acc not found; run coverage_selfhost_corpus.sh first" >&2; exit 1; }
...
for f in vibe/compiler/*_test.vibe; do
  ...
  if VIBE_COVERAGE=1 VIBE_PREOPEN_DIR="$ROOT_DIR" VIBE_FS_COMPILE=1 VIBE_SELFHOST_IMPORT_ABI=raw \
      bash "$RUNNER" --invoke cli_main "$SEED" "$f" "$bin" "__vibe_test_no_entry__" >/dev/null 2>&1 && [ -s "$bin" ]; then
    comp_ok=$((comp_ok+1))
