...
#   stage2 — scripts/generations.sh build
...
#
# Self-test:
#   scripts/test_dist_stage2_parity.sh --self-test
#     validates the vibe.abi extractor against the committed pinned seed wasm
#     (bootstrap/seed/compiler.wasm) without any build.
#
# Env:
...
self_test() {
  local seed="$PROJECT_ROOT/bootstrap/seed/compiler.wasm"
...
if [ "$REUSE_STAGE2" = "1" ] && [ -n "$STAGE2_WASM" ] && [ -f "$STAGE2_WASM" ]; then
  ...
  bash "$SCRIPT_DIR/generations.sh" build
  STAGE2_WASM="$(ls -t "$PROJECT_ROOT"/_build/selfhost/generations/*/stage2.wasm 2>/dev/null | head -n 1 || true)"
fi
