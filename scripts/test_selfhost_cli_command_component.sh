...
DEBUG_FLAG="${VIBE_SELFHOST_CLI_COMMAND_DEBUG:-0}"
...
(
  ...
  env VIBE_WASMTIME_WASM_FLAGS="$WASMTIME_WASM_FLAGS" \
    ...
    VIBE_SELFHOST_CLI_COMMAND_DEBUG="$DEBUG_FLAG" \
    "$WASMTIME_RUN" run "$COMPONENT_PATH" "$ENTRY_NAME" <"$INPUT_SOURCE" >"$OUTPUT_WASM"
)
