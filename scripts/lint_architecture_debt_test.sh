...
expect_fail
grep -qE 'src/hot.vibe:1: error ARCH999' "$TMP_ROOT/out.stderr" \
...
EOF
expect_ok
grep -qE 'ok' "$TMP_ROOT/out.stdout" || fail "drifted debt should report ok"
...
EOF
expect_fail
grep -qE 'src/hot.vibe:5: error ARCH999' "$TMP_ROOT/out.stderr" \
...
EOF
expect_fail
grep -qE 'src/other.vibe:1: error ARCH999' "$TMP_ROOT/out.stderr" \
...
EOF
expect_ok
grep -qE 'stale allowlist entry' "$TMP_ROOT/out.stderr" \
...
EOF
expect_ok --update
grep -qE '^# a hand-written note that must survive$' "$TMP_ROOT/allowlist.tsv" \
  || fail "--update dropped hand-written content"
grep -qE $'^ARCH999\tsrc/other\\.vibe\t\\*$' "$TMP_ROOT/allowlist.tsv" \
  || fail "--update dropped a file-wide exception"
grep -qE $'^ARCH999\tsrc/hot\\.vibe\t1\tlet slow = String::concat\\(acc, "x"\\)$' "$TMP_ROOT/allowlist.tsv" \
  || fail "--update did not baseline the outstanding violation"
expect_ok
grep -qE 'ok' "$TMP_ROOT/out.stdout" || fail "the regenerated baseline should be green"

# --update is idempotent: a second run reports no change and rewrites nothing.
before="$(cat "$TMP_ROOT/allowlist.tsv")"
expect_ok --update
grep -qE 'baseline already current' "$TMP_ROOT/out.stdout" \
...
grep -qE 'bad allowlist row' "$TMP_ROOT/out.stderr" \
  || fail "a line-number row should name itself as the problem"
grep -qE '#1729' "$TMP_ROOT/out.stderr" \
