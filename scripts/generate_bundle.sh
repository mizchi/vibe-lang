...
# SCRIPT_DIR, not PROJECT_ROOT: PROJECT_ROOT honors VIBE_PROJECT_ROOT, which
# generate_bundle_test.sh points at a synthetic project that has no scripts/
# of its own. Sourcing from there aborted the whole script under `set -e`
# before it did anything (`No such file or directory`), so
# `pkf run test-generate-bundle` failed unconditionally. trace_lib.sh ships
# with this script, so it is always script-relative.
...
# The cache/ dir is not guaranteed to exist: codegen_fingerprint.vibe stopped
# being tracked when the five generated artifacts moved to build-time
# generation, so a fresh clone has no lib/@vibe/compiler/cache/ at all -- and
# neither do generate_bundle_test.sh's synthetic VIBE_PROJECT_ROOT projects,
# which is why that test failed here long before the tracing helper was added
# above it.
mkdir -p "$(dirname "$CODEGEN_FP_FILE")"
