#!/usr/bin/env bash
# Generate sources_bundle.vibe — all compiler source files as string constants.
...
//
// A hash of every compiler source file in sources_manifest.tsv (the
...
if manifest_gaps:
    for target, users in sorted(manifest_gaps.items()):
        sys.stderr.write(
            ...
            + " but has no row in sources_manifest.tsv; add one or the"
