...
export interface XshInitRequest {
  prelude?: string;
  kv?: Record<string, string>;
}

export interface XshInitOkResult {
  ok: true;
  prelude: boolean;
  kv_count: number;
}

export interface XshInitErrorResult {
  ok: false;
  error: string;
}

export type XshInitResult = XshInitOkResult | XshInitErrorResult;
...
export interface XshServiceOptions {
  wasmPath?: string;
  inputPtr?: number;
  outputPtr?: number;
  outputCap?: number;
  imports?: WebAssembly.Imports;
  bootstrap?: XshInitRequest;
}

export interface XshService {
  wasmPath: string;
  instance: WebAssembly.Instance;
  memory: WebAssembly.Memory;
  rawInit(request: XshInitRequest | string): Promise<string>;
  init(request: XshInitRequest): Promise<XshInitResult>;
...
export function createXshService(
  options?: XshServiceOptions,
): Promise<XshService>;
