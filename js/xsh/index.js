...
function ensureExportFunctions(exports, wasmPath) {
  ...
  if (!exports.xsh_init) {
    throw new Error(`missing export "xsh_init" in ${wasmPath}`);
  }
  if (!exports.xsh_check) {
    throw new Error(`missing export "xsh_check" in ${wasmPath}`);
  }
  if (!exports.xsh_check_project) {
    throw new Error(`missing export "xsh_check_project" in ${wasmPath}`);
  }
  if (!exports.xsh_format) {
    throw new Error(`missing export "xsh_format" in ${wasmPath}`);
  }
  if (!exports.xsh_ide_outline) {
    throw new Error(`missing export "xsh_ide_outline" in ${wasmPath}`);
  }
  if (!exports.xsh_ide_peek_def) {
    throw new Error(`missing export "xsh_ide_peek_def" in ${wasmPath}`);
  }
  if (!exports.xsh_ide_search) {
    throw new Error(`missing export "xsh_ide_search" in ${wasmPath}`);
  }
}
...
export async function createXshService(options = {}) {
  ...
  const wasmInit = exports.xsh_init;
  const wasmCheck = exports.xsh_check;
  const wasmCheckProject = exports.xsh_check_project;
  const wasmFormat = exports.xsh_format;
  const wasmIdeOutline = exports.xsh_ide_outline;
  const wasmIdePeekDef = exports.xsh_ide_peek_def;
  const wasmIdeSearch = exports.xsh_ide_search;
  ...
  if (options.bootstrap) {
    ...
    if (result.ok !== true) {
      const detail = typeof result.error === "string"
        ? result.error
        : "unknown error";
      throw new Error(`xsh_init failed: ${detail}`);
    }
  }
