...
use xs_core::{BuiltinRegistry, Environment, Literal, Value};
    ...
    fn eval_ir(&mut self, ir: &TypedIrExpr, env: &Environment) -> Result<Value, RuntimeError> {
        match ir {
            ...
            TypedIrExpr::Var { name, .. } => env
                .lookup(&xs_core::Ident(name.clone()))
                .cloned()
            ...
            TypedIrExpr::Lambda { params, body, .. } => {
                // Create closure
                let param_names: Vec<xs_core::Ident> = params
                    .iter()
...
mod tests {
    ...
    use xs_core::ir::TypedIrExpr;
    use xs_core::{Expr, Ident, Literal, Type};
