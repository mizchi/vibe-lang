...
fn test_str_concat() {
    ...
    let (stdout, stderr, success) = run_xsc(&["run", "test_str_concat.xs"]);
...
fn test_int_to_string() {
    ...
    let (stdout, stderr, success) = run_xsc(&["run", "test_int_to_string.xs"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("\"42\""));

    let code = "(int-to-string -123)";
    fs::write("test_int_to_string_neg.xs", code).unwrap();
    let (stdout, stderr, success) = run_xsc(&["run", "test_int_to_string_neg.xs"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("\"-123\""));

    let code = "(int-to-string 0)";
    fs::write("test_int_to_string_zero.xs", code).unwrap();
    let (stdout, stderr, success) = run_xsc(&["run", "test_int_to_string_zero.xs"]);
    assert!(success, "Run failed: {stderr}");
...
fn test_string_to_int() {
    ...
    let (stdout, stderr, success) = run_xsc(&["run", "test_string_to_int.xs"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("123"));

    let code = r#"(string-to-int "-456")"#;
    fs::write("test_string_to_int_neg.xs", code).unwrap();
    let (stdout, stderr, success) = run_xsc(&["run", "test_string_to_int_neg.xs"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("-456"));

    let code = r#"(string-to-int "0")"#;
    fs::write("test_string_to_int_zero.xs", code).unwrap();
    let (stdout, stderr, success) = run_xsc(&["run", "test_string_to_int_zero.xs"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("0"));
}
...
fn test_string_to_int_error() {
    ...
    let (_stdout, _stderr, success) = run_xsc(&["run", "test_string_to_int_err1.xs"]);
    assert!(!success, "Should have failed");

    let code = r#"(string-to-int "")"#;
    fs::write("test_string_to_int_err2.xs", code).unwrap();
    let (_stdout, _stderr, success) = run_xsc(&["run", "test_string_to_int_err2.xs"]);
    assert!(!success, "Should have failed");
}
...
fn test_string_length() {
    ...
    let (stdout, stderr, success) = run_xsc(&["run", "test_string_length_empty.xs"]);
    assert!(success, "Run failed: {stderr}");
    assert!(stdout.contains("0"));

    let code = r#"(string-length "Hello, World!")"#;
    fs::write("test_string_length_long.xs", code).unwrap();
    let (stdout, stderr, success) = run_xsc(&["run", "test_string_length_long.xs"]);
    assert!(success, "Run failed: {stderr}");
...
fn test_string_operations_combined() {
    ...
    let (stdout, stderr, success) = run_xsc(&["run", "test_string_ops_combined1.xs"]);
    assert!(success, "Run failed: {stderr}");
    ...
    let (stdout, stderr, success) = run_xsc(&["run", "test_string_ops_combined2.xs"]);
    assert!(success, "Run failed: {stderr}");
