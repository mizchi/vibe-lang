...
mod core_tests {
    ...
    #[test]
    fn test_compose() {
        let code = r#"(((lambda (f g) (lambda (x) (f (g x)))) (lambda (x) (+ x 1)) (lambda (x) (* x 2))) 5)"#;
    ...
    #[test]
    fn test_flip() {
        let code = r#"(((lambda (f) (lambda (x y) (f y x))) (lambda (x y) (- x y))) 3 10)"#;
    ...
    fn test_boolean_ops() {
        let code = r#"(list 
  ((lambda (b) (if b false true)) true)
  ((lambda (a b) (if a b false)) true false)
  ((lambda (a b) (if a true b)) false true))"#;
...
mod list_tests {
  ...
  (match xs
    ((list) (list))
    ((list h t) (cons (f h) (map f t))))) (lambda (x) (* x 2)) (list 1 2 3))"#;
  ...
  (match xs
    ...
    ((list h t) 
      (if (p h)
          (cons h (filter p t))
          (filter p t))))) (lambda (x) (> x 2)) (list 1 2 3 4))"#;
...
mod math_tests {
  ...
  (if (= b 0)
    ...
    fn test_numeric_predicates() {
        let code = r#"(list 
  ((lambda (n) (= (% n 2) 0)) 4)
  ((lambda (n) ((lambda (b) (if b false true)) ((lambda (n) (= (% n 2) 0)) n))) 5))"#;
...
mod integration_tests {
    ...
    fn test_list_sum_with_fold() {
        let code = r#"((lambda (xs) ((rec fold-left (f acc xs)
  (match xs
    ((list) acc)
