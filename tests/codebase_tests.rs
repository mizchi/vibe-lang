...
fn test_function_hashing() {
    ...
    let func1_code = r#"(lambda (x) (+ x 1))"#;
    let func2_code = r#"(lambda (x) (+ x 1))"#;
...
fn test_content_addressed_storage() {
    ...
    let unique_func = r#"(lambda (x y z) (+ (* x y) z))"#;
...
fn test_nested_lambda() {
    // Test nested lambdas
    let code = r#"((lambda (x) ((lambda (y) (+ x y)) 20)) 22)"#;
...
fn test_module_syntax() {
    // Test module syntax
    let code = r#"(module Math (export id) (let id (lambda (x) x)))"#;
