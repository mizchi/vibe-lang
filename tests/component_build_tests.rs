...
fn create_test_module(dir: &TempDir, name: &str, content: &str) -> PathBuf {
    let file_path = dir.path().join(format!("{}.xs", name));
...
(module Math
  ...
  (let multiply (fn (x y) (* x y))))"#;
    ...
    let output = Command::new("cargo")
        .args(&["run", "-p", "cli", "--bin", "xsc", "--", "component", "build"])
        .arg(&module_path)
        .arg("-o")
        .arg(&output_path)
        .arg("--version")
...
(module InvalidModule
  ...
  (let add (fn (x y) (+ x y))))"#;
    ...
    // This should fail
    assert!(!output.status.success(), "Build should have failed for invalid module");
...
(module StringOps
  ...
  (let capitalize (fn (s) s)))"#;  // Simplified for testing
    ...
    let wit_output = Command::new("cargo")
        .args(&["run", "-p", "cli", "--bin", "xsc", "--", "component", "wit"])
        .arg(&module_path)
        .output()
