...
このディレクトリは、`src/lib` から生成した wasm-gc 成果物を Deno で `clients/js/index.js` バインディング経由で検証する統合テストです。
...
- `clients/js/index.js` バインディング経由で呼び出す（`createVibeService({ bootstrap })` / `service.init(...)` で prelude + kv 初期注入対応）
