...
fn test_three_parameter_currying() {
    ...
    let code = r#"(((lambda (x y z) (+ (* x y) z)) 3 4) 5)"#;
...
fn test_composition_with_currying() {
    ...
    let code = r#"(((lambda (f g) (lambda (x) (f (g x)))) (+ 1)) (* 2))"#;
...
fn test_higher_order_currying() {
    ...
    let code = r#"(((lambda (f x y) (f x y)) +) 3 4)"#;
...
fn test_curried_type_checking() {
    ...
    let code = r#"((lambda (x:Int y:Int) (+ x y)) 5)"#;
